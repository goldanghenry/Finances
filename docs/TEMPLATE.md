...
> **저자·편집용** 신규 편 템플릿 — 사이트 메뉴·검색에는 없습니다.
...
| 난이도 | L3 (Deep) — [READER-GUIDE §L등급](READER-GUIDE.md) |
| 예상 읽기 시간 | N분 |
...
| 항목 | 내용 |
|------|------|
| **난이도** | L3 — [READER-GUIDE §L등급](READER-GUIDE.md) |
| **선수** | (없음 / [문서](path.md)) |
| **이번 편에서 쓰는 기호** | (없음 / 약어 나열) |
| **복습 한 줄** | (해당 시 한 문장) |
...
**순서**: (1) **변수 설명표** → (2) LaTeX 수식 → (3) 기호 예시(구체 금액 없음).
...
```markdown
!!! info "약어 (English, 한글)"
    한 줄 정의. [glossary](../00-roadmap/glossary.md#anchor) · 선수 문서 링크.
```
...
??? note "정답 힌트"
    ...
    (정답·수식은 여기. HTML `<details>` 대신 이 형식 — GitHub Pages에서 LaTeX 렌더링)
