...
쉽게 말하면: 3부 리그 축구팀은 다음 시즌에 승격 기회가 있지만, 코스닥 관리군 기업은 상장폐지 심사 대상이 될 수 있다. 실제로 2026년부터 강화된 퇴출 규정(동전주·시총 미달·자본잠식 등)이 적용되면 관리군 편입 = 투자 자금이 묶이거나 상장폐지로 전액 손실 가능한 상황이 올 수 있다.

핵심은: 프리미엄 세그먼트 편입 예상 종목들은 제도 도입 전후로 **기관·연기금 자금 유입 기대**가 높아져 주가가 움직일 수 있다. 반면 관리군 편입이 예상되는 종목은 **기관 매도 압력**이 커질 수 있다. 이것이 개인 투자자가 이 제도를 알아야 하는 실질적 이유다.

실제 투자에서는 이렇게 씁니다: 코스닥 소형주를 보유 중이라면, 해당 종목이 프리미엄·스탠더드·관리군 중 어디로 분류될 가능성이 높은지 재무 기준(시총, 순이익, 자본잠식 여부)으로 점검해야 한다. 주의할 점: 세그먼트 제도가 도입 전이므로 현재 보도 기준으로는 “어떤 종목이 프리미엄에 들어간다”는 확정이 아니다. 제도 확정 후 공식 발표를 반드시 확인해야 한다.
...
flowchart TD
  Q{"개별 코스닥 직접투자"}
  Q -->|"예 소수"| B4["Bucket4 상한·리스크체크"]
  Q -->|아니오| Idx["지수 ETF 코어"]
  B4 --> Check["관리군 적자 동전주 공시"]
```
