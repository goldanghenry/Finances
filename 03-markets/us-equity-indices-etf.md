...
|  \(원화\)  |  원화  | 본문 §4·위 식 맥락 참고 |
|  \(달러\)  |  달러  | 본문 §4·위 식 맥락 참고 |
...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(E_{\text{Mag7}}\) | E \textMag7 | 본문 §4·위 식 맥락 참고 |
| \(w_{\text{S&P}}\) | w \textS&P | 본문 §4·위 식 맥락 참고 |
| \(s_{\text{S&P}}\) | s \textS&P | 본문 §4·위 식 맥락 참고 |
| \(w_{\text{QQQ}}\) | w \textQQQ | 본문 §4·위 식 맥락 참고 |
| \(s_{\text{NDX}}\) | s \textNDX | 본문 §4·위 식 맥락 참고 |
| \(w_{\text{개별}}\) | w \text개별 | 본문 §4·위 식 맥락 참고 |
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
\(s\): 각 바스켓의 Mag7 비중(시점별 변동), \(w\): 포트 비중. \(w_{\text{S&P}}=50\%, w_{\text{QQQ}}=50\%\)이면 **broad 2개**만으로도 Mag7 **이중**.

**장기 수익 분해** ([etf-index-funds.md](etf-index-funds.md)와 동일 프레임):

| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(R_{\text{투자자}}\) | R \text투자자 | 본문 §4·위 식 맥락 참고 |
| \(R_{\text{지수}}\) | R \text지수 | 본문 §4·위 식 맥락 참고 |
...
|---|---------------|---------------|
| 100만 달러, 지수 연 8% (가상) | 20년 후 **약 432만 달러** | **약 448만 달러** |
| **차이(가상)** | — | **약 +16만 달러** |
