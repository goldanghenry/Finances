...
```mermaid
flowchart LR
  Inv[투자자] -->|환매청구| Fund[뮤추얼펀드_운용사]
  Fund -->|기준가_체결| NAV[일일_NAV]
...
```mermaid
flowchart LR
  R2[높은_R2] --> Suspect[클로짓_의심]
  AS[낮은_ActiveShare] --> Suspect
  Fee2[높은_TER] --> Suspect
  Suspect --> Action[교체_인덱스ETF_검토]
```
...
|   \(A_T\)   | A T | A T이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|   \(A_0\)   | A 0 | A 0이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
|  \(w_{p\)  |  w p  | \(w p\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(w_{b\)  |  w b  | \(w b\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
\(w_p\): 펀드 비중, \(w_b\): 벤치 비중. **AS 낮음** + **높은 보수** → **클로짓** 의심| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
...
### 6.4 정보 비율 (Information| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
