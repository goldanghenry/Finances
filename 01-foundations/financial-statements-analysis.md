...
|------|------|----------------|
| \(유동비율\) | Current ratio | 유동자산 ÷ 유동부채 |
|  \(유동자산\)  |  유동자산  | 본문 §4·위 식 맥락 참고 |
|  \(유동부채\)  |  유동부채  | 본문 §4·위 식 맥락 참고 |
|  \(당좌비율\)  |  Quick ratio  | 본문 §4·위 식 맥락 참고 |
|  \(재고\)  |  재고  | 본문 §4·위 식 맥락 참고 |
|  \(선급비용 등\)  |  선급비용 등  | 본문 §4·위 식 맥락 참고 |
\[
...
\[
ROIC \approx \frac{\text{NOPAT}}{\text{투하자| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
\underbrace{\frac{\text{매출}}{\te| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |

xt{평균자산}}}_{\text{자산회전율}} \times
\underbrace{\frac{\text{평균자산}}{\text{평균자본}}}_{\text{자본배수}}
\]


**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
**5단** (교육용): 세금·이자 부담을 분리해 *| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
| \(ROE\) | ROE | 본문 §4·위 식 맥락 참고 |
...
*영업이익률 × 총자산회전 × 재무레버리지 × 이자부담 × 세율** 형태로 확장 가능 — **금리 상승기**([macro-02](../02-economics/macro-02-money-inflation.md))에는 **이자 부담 요인**이 ROE를 직접 깎는다.
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
| \(OCF\) | 영업현금흐름 | 영업활동에서 발생한 현금 |

다.
**발생주의(Accruals) 근사** (교육용, 부| 기호 | 이| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
| \(OCF\) | 영업현금흐름 | 영업활동에서 발생한 현금 |
...
**읽는 법**: 위 식의 기호는 바| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
**Sloan(1996) 직관**: 높은 **총발생주의| | 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(FCF\) | 잉여현금흐름 | 투자자에게 가용한 현금 |
| \(OCF\) | 영업현금흐름 | 영업활동에서 발생한 현금 |
...
| \(OCF\) | 영업현금흐름 | 영업활동에서 발생한 현금 |
 ...
 이익** 비중은 이후 **수익률 하향**과 연관된다는 실증 — **인과 단정 금지**, **경고등**으로 사용.
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
배당·상환 여력은 [cash-flow-basics| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |

](cash-flow-basics.md)와 연계. **성장 CAPEX**가 큰 [battery-lfp-ncm-ess](../03-markets/sectors/battery-lfp-ncm-ess.md)는 **FCF 마이너스**가 “나쁨”만은 아니다 — **투자 주기·수주**와 함께 본다.

### 6.6 활동성·현금전환주기

\[
\text{재고회전율} = \frac{\text{매출원가}}{\text{평균재고}}, \q| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
