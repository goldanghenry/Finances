...
| 이유 | 설명 |
|------|------|
| **FV 입력** | 복리 공식의 PMT는 “실제로 넣은 돈” — 수익률만큼 중요 |
...
### 예제 1: A vs B — 같은 연봉, 다른 저축률

| | 가상 A | 가상 B |
|--|--------|--------|
| 연봉(세후) | 4,800만 원 | 4,800만 원 |
| 연 지출 | 3,360만 원 | 4,320만 원 |
| **저축률** | **30%** | **10%** |
| 연 저축 | 1,440만 원 | 480만 원 |

10년, r=6% 가정 시 FV는 A가 B의 **약 3배**에 가깝다(교육용) — **수익률을 맞춰도 저축률이 지배**.

### 예제 2: 가상 C — 연봉 인상 + 라이프스타일 크립
...
승진했지만 FV 입력(PMT)은 **감소** — “번 것 같지 않다”.
...
ISA 50만 + 비상금 30만 = **저축률 약 25%** (320 기준).

### 예제 4: 보너스 800만 원 (가상 E)

| 배분 | 금액 | 이유 |
|------|------|------|
| 비상금 보강 | 200만 | Bucket 0 미달 |
| 카드 상환 | 150만 | 18% — [debt-and-interest](debt-and-interest.md) |
| ISA 일시 | 300만 | 한도 내 |
| 여가 | 150만 | 번아웃 방지 |

### 예제 5: 연말정산 환급 120만 원 (가상 F)
