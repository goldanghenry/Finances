...
3. Bucket은 “비중 %”보다 **채우는 순서**가 먼저: 0 비상금 → 1 정책 → 2 [ISA](../06-korea-policy/isa.md)/[IRP](../06-korea-policy/irp.md) → 3 코어 — [time-horizon-and-buckets](../04-portfolio/time-horizon-and-buckets.md).
...
**쉽게 말하면:** 아무리 좋은 투자 전략을 알고 있어도 매달 "남는 돈"이 없으면 실행할 수 없다. 현금흐름 관리는 투자 지식보다 **먼저 갖춰야 할 기반**이다.

**이 개념을 모르면 생기는 실수:**
- 승진·연봉 인상 후 "이제 투자해야지"라고 생각했는데 지출도 같이 늘어 남는 돈은 그대로인 **라이프스타일 크립**에 빠진다.
- "투자 수익률 10%" 목표를 세우면서도 정작 **매달 얼마를 입금하는지(PMT)** 모른다.
- 카드 명세서를 확인 안 하다가 ISA 납입 여력이 없어 **연간 세제 혜택을 통째로 날린다**.
...
**핵심은:** 투자 앱의 수익률보다 **통장·카드 흐름**을 먼저 파악해야 장기 목표에 도달할 확률이 올라간다.
...
**수도꼭지와 배수구**: 소득이 꼭지, 지출이 배수구다. **배수구(고정비·구독·이자)가 크면 탱크(저축·투자)는 안 찬다.** 예를 들어, 직장인 A씨가 매달 월급을 받지만 OTT 구독 3개·배달앱·할부금이 겹쳐 있으면 ISA 납입 여력이 남지 않는다. 배수구부터 줄이는 것이 수익률 높은 상품을 찾는 것보다 먼저다.

**파이프라인**: 월급 → **세금·4대보험** → **생활비** → **부채 이자** → **남는 물**만 Bucket으로 간다. **앞 단계에 구멍이 있으면 뒤 Bucket은 말뿐**이다. 예를 들어 직장인 B씨는 명목 연봉 기준으로 ISA 납입 계획을 세웠다가 실수령이 훨씬 작아 계획이 공중에 뜬 경험을 한다.

**회사와 가계**: 회사도 "이익은 났는데 통장이 비었다"가 있다 — 현금흐름표가 그걸 설명한다([financial-statements-intro](financial-statements-intro.md)). 가계도 마찬가지다. 연말정산 환급금을 받았는데 이미 소비에 써버린 경우 "소득이 늘었는데 저축이 안 늘어요"가 된다.
...
**신용카드 = 다음 달 유출**: 결제일을 급여일 **다음 주**로 맞추면, 한 달치 지출이 **두 달에 겹쳐 보이는** 착시가 생긴다. 예를 들어 직장인 C씨가 1월에 쓴 카드값이 2월 결제일에 나가면 "2월에 과소비했다"는 착각을 할 수 있다. 현금흐름표는 **발생이 아니라 현금 이동 시점** 기준으로 써야 이 착시를 막는다.
...
flowchart TD
  ...
  Net --> Save[순저축]
  Save --> B0[Bucket0]
  Save --> B1["Bucket1 청년도약등"]
  Save --> B2["Bucket2 ISA IRP"]
  Save --> B3["Bucket3 코어"]
```
...
```mermaid
flowchart TD
  Debt["고금리 부채"] --> Int["이자 유출"]
  Int --> Less["순저축 감소"]
  Less --> Delay["Bucket 지연"]
```
...
> **해석**: \((B+T)/M\)이 높을수록 장기 자산 축적 속도가 빠르다. 저축률 25%이면 4년에 약 1년치 소득을 순자산으로 쌓는 반면, 10%이면 10년이 걸린다. **M을 본인 수치로 대입**해 Bucket 0→3 도달 시점을 가늠해 보자.
...
**Q. 저는 ISA를 이미 쓰고 있는데, 현금흐름 관리와 어떻게 연결하나요?**  
**A.** ISA 납입 한도(연간)를 월 분할하면 **PMT 형태의 고정 저축선**이 된다. 월 현금흐름표에서 "ISA 자동이체"를 먼저 뺀 뒤 생활비를 설계하면 납입 일관성이 높아진다. 세제 혜택을 놓치지 않으려면 [isa](../06-korea-policy/isa.md) 연간 한도를 확인한다.

**Q. 매달 저축이 불규칙한데 어떻게 관리하나요?**  
**A.** **쉽게 말하면:** 금액보다 **이체 행위의 규칙성**이 더 중요하다. 금액이 달라도 매월 첫째 주 금요일에 이체하는 습관을 갖추면 장기 누적 효과가 훨씬 안정적이다. [rebalancing-and-dca](../04-portfolio/rebalancing-and-dca.md) 참고.
