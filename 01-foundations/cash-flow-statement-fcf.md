...
| \(\X_\text{ gross}\) | X  gross | 본문 §4·위 식 맥락 참고 |
...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(\text{CAPEX}\) | CAPEX | 투자활동 현금 유출(취득) |
| \(\text{Sales}\) | 매출 | 분모·규모 정규화용 |
| \(\text{OCF}\) | 영업현금흐름 | 영업활동 현금 창출 |
...
**읽는 법**: **T**와 **D**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **T**, **D**, **A**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
...
**읽는 법**: **PV**와 **T**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **PV**, **T**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
또는 **순이익** 기반:
...
**읽는 법**: **PV**와 **D**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **PV**, **D**, **A**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
...
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
|------|------|----------------|
owing**: **차입 − 상환** (재무활동). **레버리지** 확대 시 **FCFE**는 **단기** 늘 수 있으나 **
| 이름 | 이 식에서 의미 | |
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(WACC\) | 가중평균자본비용 | 기업·프로젝트 할인율 근사 |
...
**유도 (L4)**:
1. **정의**: **WACC**, **PV**, **CFF_t**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
...
(분류)** 반영 — **크레딧**·**주식** 모두 **OCF/부채**, **FCF/이자** 봄.
