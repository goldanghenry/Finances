...
|  **VI. 이사회 등의 의견**  |  배당·사채 등  | 본문 §4·위 식 맥락 참고 |
...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
|  \(자산\)  |  자산  | 본문 §4·위 식 맥락 참고 |
|  \(부채\)  |  부채  | 본문 §4·위 식 맥락 참고 |
|  \(자본\)  |  자본  | 본문 §4·위 식 맥락 참고 |
\[
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
- **지속적으로** \( \text{OCF} / \text{NI} < 1 \) 이면| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
\[
\text{이자보상배율} = \frac{\text{영업이익 | 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r_{\text{변동}}\) | r \text변동 | 본문 §4·위 식 맥락 참고 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |

\[
\Delta \text| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
\[
\text{DSO (일)} = \frac{\text{매출채권}}{| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
frac{\text{순이익}}{\text{자본}} = \underbrace{\frac{\text{순이익}}{\text{매출}}}_{\text{순이익률}} \times \underbrace{\frac{\text{매출}}{\text{자산}}}_{\text{자산회전}} \times \underbr| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
| \(ROE\) | ROE | 본문 §4·위 식 맥락 참고 |
| \(PER\) | 주가수익비율 | 주가 ÷ EPS |
...
\[
PER = \frac{P}{\text{EPS}}, \quad \text{EPS}_{\text{조정}} = \frac{\text{순이익} - \text{일회성} + \text{IR조정항}| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PER_{\text{조정}}\) | PER \text조정 | 본문 §4·위 식 맥락 참고 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
| \(EPS\) | 주당순이익 | 순이익 ÷ 발행주식수 |
| \(P\) | 포트 규모 | 가상 포트폴리오 규모(만 원) |

\[
PER_{\text{조정}} = \fra| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
| \(EPS\) | 주당순이익 | 순이익 ÷ 발행주식수 |
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
IR의 “조정 EPS”가 **주석에 없는 항목**을 빼면 \( PER_{\text{조정}} \ll PER_{\text{공시}} \) 착시 — [financial-statements-intro](financial-statements-i| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(t_{\text{lag}}\) | t \textlag | 본문 §4·위 식 맥락 참고 |
| \(t_0\) | t 0 | 본문 §4·위 식 맥락 참고 |
| \(t_1\) | t 1 | 본문 §4·위 식 맥락 참고 |
| \(t_2\) | t 2 | 본문 §4·위 식 맥락 참고 |

ntro.md) 예제 4.

### 6.7 공시 지연과 정보 가| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(t_{\text{lag}}\) | t \textlag | 본문 §4·위 식 맥락 참고 |
| \(t_0\) | t 0 | 본문 §4·위 식 맥락 참고 |
| \(t_1\) | t 1 | 본문 §4·위 식 맥락 참고 |
...
정보가 시장에 반영되는 시점 \( t_0 \), 공시 제출| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(t_{\text{lag}}\) | t \textlag | 본문 §4·위 식 맥락 참고 |
| \(t_1\) | t 1 | 본문 §4·위 식 맥락 참고 |
| \(t_2\) | t 2 | 본문 §4·위 식 맥락 참고 |
...
**읽는 법**: 위 | 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
| \(m_i\) | m i | 본문 §4·위 식 맥락 참고 |
| \(OP_i}\) | OP i | 본문 §4·위 식 맥락 참고 |
| \(Rev_i}\) | Rev i | 본문 §4·위 식 맥락 참고 |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |
| \(P\) | 포트 규모 | 가상 포트폴리오 규모(만 원) |
