...
|             \(varepsilon\)             | varepsilon | varepsilon이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
|  \(lon_i\)  |  lon i  | lon i이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
|  \(\sum_i\)  |  sum i  | sum i이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(DD_t\) | 드로다운 | 고점 대비 현재 하락률 |
| \(P_t\) | t시점 가격 | 포트·지수 수준 |
| \(\max_{s\le t} P_s\) | 러닝 최고가 | t까지의 최고점 |
...
**읽는 법**: **DD_t**와 **P_t**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **DD_t**, **P_t**, **max_**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
...
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(w_i\)  |  w i  | w i이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(gma_i}\)  |  gma i  | \(gma i\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
**리스크 패리티** (개념): 각 자산이 **포트 σ 에 | 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
...
- [behavioral-finance-complete.md| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
|  \(VaR_{95\)  |  VaR 95  | \(VaR 95\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
](.| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
