...
flowchart TD
  Phil[투자_철학]
  Phil --> CoreP[코어_패시브_80to100%]
  Phil --> SatA[위성_액티브_0to20%]
  CoreP --> ISA[ISA_IRP_QQQ_글로벌]
  SatA --> Learn[섹터_학습_소수매매]
  SatA --> QLD[QLD_한정]
  DB[DB_2a] -.->|선택불가| X[패시브액티브_무관]
```
...
```mermaid
flowchart LR
  Study[섹터_5단계_공부] --> Filter[뉴스_IR_필터]
  Filter --> CoreKeep[코어_ETF_유지]
  Filter --> SatOnly[위성만_종목]
  CoreKeep --> B3[Bucket3]
  SatOnly --> B4[Bucket4_상한20%]
```
...
|             ETF             | ETF | ETF이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|             approx             | approx | approx이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|             E             | E | E이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|             alpha             | alpha | alpha이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|             quad             | quad | quad이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|             leveraged             | leveraged | leveraged이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|             etf             | etf | etf이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
