...
| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(eta_p\)  |  eta p  | eta p이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(w_i\)  |  w i  | w i이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(eta_i\)  |  eta i  | eta i이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
**읽는 법**: 시장 초과수익에 대한 민감도가 **β**다. **R_f**·**ERP**와 함께 요구수익 **r**을 구성한다. [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 참고.**예**: \(R_f=3\%\), \(E(R_m)=8\%\), \(\beta=1.2\) → \(E(R)=3+1.2\times5=9\%\).
