...
퀀트 워크플로는 **ML 제품 파이프라인**과 **형태가 비슷**하다. 차이는 **데이터가 노이즈가 크고**, **레이블(미래 수익)이 비정상**이며, **배포(실전 매매) 비용**이 크다는 점이다.

| ML / 데이터 엔지니어링 | 퀀트 투자 | 금융 함정 |
|------------------------|-----------|-----------|
| **원천 DB** (로그·이벤트) | 가격·재무·공시(DART) | **수정·상폐·생존편향** |
| **ETL** | 클린징·조정·통화 통일 | **미래 정보 누수**(look-ahead) |
| **피처 스토어** | P/B·모멘텀·변동성 | **정의 불일치**(GAAP vs IFRS) |
| **학습/검증 분할** | In-sample / OOS | **레짐 변화**(금리·규제) |
| **하이퍼파라미터 튜닝** | 규칙·임계값 그리드 서치 | **과최적화** |
| **오프라인 A/B** | 백테스트 | **거래비용·세금 무시** |
| **온라인 서빙** | 실전 매매·슬리피지 | **용량·유동성** |
| **모니터링** | 성과·드리프트 | **팩터 프리미엄 소멸** |
...
```mermaid
flowchart TD
  Passive[패시브_시장가중] --> Core[코어_Bucket3]
  Smart[스마트베타_규칙팩터] --> Sat[보조_또는_위성_상한]
  Active[액티브_채권선택] --> Hard[지속_α_어려움]
```
