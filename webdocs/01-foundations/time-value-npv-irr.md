...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(C_0\) | 초기 투자 | 시점 0 유출(투자금). 부호 규칙은 모형마다 통일 |
...
\(C_0\): 시점 0 유출(초기 투자, 양수로 표기 시 부호 주의). **독립·상호배타** 판단의 **1순위 지표**.
...
**IRR**은 위 방정식의 \(r\) 해. **비선형**이라 해가 0개·복수일 수 있다(부호가 여러 번 바뀌는 CF). 실무는 **MIRR**·**NPV 프로필** 병행.
...
\(r_e\): 자기자본 비용, \(r_d\): 부채 비용, \(T_c\): 법인세율. [macro-06](../02-economics/macro-06-asset-prices-macro.md)에서 \(R_f\), ERP가 **\(r_e\)** 를 움직인다.
...
\(C\): 매기간 동일 CF, \(n\): 기간. **채권 쿠폰 스트립**, **연금저축**, **전세 보증 반환 없는 월세** 모델에 사용.
...
**주식** [stocks-equities-intro](../03-markets/stocks-equities-intro.md): \(C_1 = D_1\) (다음 배당). \(r \uparrow\) 또는 \(g \downarrow\) → **PV ↓** → 주가 압력.
...
\(C\): 쿠폰, \(F\): 액면(만기 원금), \(y\): **YTM**(시장 할인율). **\(y \uparrow \Rightarrow P_{bond} \downarrow\)** — [bonds-fixed-income](../03-markets/bonds-fixed-income.md).
...
\(TV\): 종료가치, 종종 **Gordon** \(FCF_{T+1}/(WACC-g)\). 상세는 [cash-flow-statement-fcf](cash-flow-statement-fcf.md).
...
|------|-----|
| 카드 잔액 | 500만 원 |
| 연 이자(확실) | 18% → **1년 이자 90만 원** |
| 주식 기대수익(불확실) | 10% on 500만 → **50만 원** |

**NPV 관점**: 확실 **이자 절감** 90만 > 기대 50만 → **상환**이 hurdle 대비 우월. [debt-and-interest](debt-and-interest.md), [cash-flow-basics](cash-flow-basics.md)와 연결.
