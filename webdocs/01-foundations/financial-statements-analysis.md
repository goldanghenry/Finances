...
| \(유동비율\) | Current ratio | 유동자산 ÷ 유동부채 |
|   \(유동자산\)   | \(유동자산\) | \(유동자산\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|   \(유동부채\)   | \(유동부채\) | \(유동부채\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|   \(당좌비율\)   |   Quick ratio   | \(당좌비율\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|   \(재고\)   | \(재고\) | \(재고\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|   \(선급비용 등\)   | \(선급비용 등\) | \(선급비용 등\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
**읽는 법**: 위 식의 각 기호는 직전 변수표와 §4 정의에 대응한다. 구체 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
**유도 (L4)**:
1. **정의**: 기호를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동*| 기호 | 이름 | 이 식에서 의미 |
|      ------      | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
*이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
투하자본은 **유형·무형·운전자본 − 비이자부채** 등 정의가 제각각 — **동일 정의로 시계열·동종 비교**.
...
**읽는 법**: 위 식의 각 기호는 직전 변수표와 §4 정의에 대응한다. 구체 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
**유도 (L4)**:
1. **정의**: 기호를 동일 시점·동일 통화로 맞춘다. — 단위 불| 기호 | 이름 | 이 식에서 의미 |
|      ------      | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
**5단** (교육용): 세금·이자 부담을 분리해 **영업이익률 × 총자산회전 × 재무레버리지 × 이자부담 × 세율** 형태로 확장 가능 — **금리 상승기**([macro-02](../02-economics/macro-02-money-inflation.md))에는 **이자 부담 요인**이 ROE를 직접 깎는다.
...
1. **정의**: 기호를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제| 기호 | 이름 | 이 식에서 의미 |
|      ------      | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |

 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
### 6.5 이익의 질 (Quality of Earnings)
...
**유도 (L4)**:
1. **정의**: 기호를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는| 기호 | 이름 | 이 식에서 의미 |
|      ------      | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
 ...
 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
**발생주의(Accruals) 근사** (교육용, 부호 주의):

\[
\text{Accruals} \approx \text{당기순이익} - OCF
\quad \text{또는} \quad
\frac{\Delta \text{운전자본}}{\text{매출}} \text{ 추세}
\]


**읽는 법**: 위 식의 각 기호는 직전 변수표와 §4 정의에 대응한다. 구체 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
**유도 (L4)**:
1. **정의**: 기호를 동일 | 기호 | 이름 | 이 식에서 의미 |
|      ------      | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(FCF\) | 잉여현금흐름 | 투자자에게 가용한 현금 |
| \(OCF\) | 영업현금흐름 | 영업활동에서 발생한 현금 |

시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
**Sloan(1996) 직관**: 높은 **총발생주의 이익** 비중은 이후 **수익률 하향**과 연관된다는 실증 — **인과 단정 금지**, **경고등**으로 사용.
...
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
배당·상환 여력은 [cash-flow-basics](cash-flow-basics.| 기호 | 이름 | 이 식에서 의미 |
|  ------  | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |

md)와 연계. **성장 CAPEX**가 큰 [battery-lfp-ncm-ess](../03-markets/sectors/battery-lfp-ncm-ess.md)는 **FCF 마이너스**가 “나쁨”만은 아니다 — **투자 주기·수주**와 함께 본다.

### 6.6 활동성·현금전환주기

\[
\text{재고회전율} = \frac{\text{매출원가}}{\text{평균재고}}, \quad
\text{매출채권회전율} = \frac{\text{매출| 기호 | 이름 | 이 식에서 의미 |
|  ------  | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
...
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
 ...
 기호를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
\[
