...
**이 개념을 모르면 생기는 실수:**
- 신용카드 결제일과 사용일을 혼동해서 월별 지출 집계가 계속 어긋났다.
- 가계부를 너무 세밀하게 쓰려다가 3개월 만에 포기했다.
- 가계부 없이 투자를 시작했다가 "매달 얼마 납입할 수 있는지" 모른 채 ISA/IRP 납입 계획을 세웠다.
...
**영수증 상자 vs 통장**: 상자에 넣는 날은 **발생**(쇼핑한 날), 통장에서 빠지는 날은 **현금**(카드 결제일). 가계 **현금흐름**은 통장 기준이 자연스럽다. 예를 들어, 직장인 A씨가 1월 29일에 쇼핑했는데 카드 결제일은 2월 10일이면, 이 지출은 2월 현금흐름에 잡힌다. 어느 기준을 쓸지 처음부터 정하고 12개월 동안 유지하는 것이 핵심이다.

**색깔 태그**: 수입·지출 항목마다 **고정 색**을 쓰면 "이번 달 왜 투자가 줄었지?"가 한눈에 보인다. 쉽게 말하면, 색깔이 다르면 어디서 돈이 새는지 바로 보인다.

**주방 재고**: 냉장고에 넣은 날(발생)과 계산대에서 결제한 날(현금)이 다르면, "이번 주 식비"를 두 번 세게 된다. **한 기준을 정해 꾸준히 쓰는 것이 정확도 100%보다 중요하다.**

**80% 채운 달력**: 운동 앱에서 하루 빠뜨려도 **한 달 24일**이면 습관은 유지된다. 가계도 **전 거래의 80%**만 분류해도 **총지출·저축 이체**는 잡힌다. 예를 들어, 직장인 B씨가 "완벽하게 다 기록해야 해"라는 압박에 3개월 만에 가계부를 그만뒀지만, **주요 이체·카드 합계만 잡는 80% 규칙**으로 다시 시작해 1년을 유지했다.
...
```mermaid
flowchart LR
  Bank["통장 카드 내역 다운로드"] --> Raw["탭1 원장"]
  Raw --> Rule["분류규칙 키워드"]
  Rule --> Month["탭2 월집계"]
  Month --> Year["탭3 연간 YTD"]
  Year --> Action["Bucket 자동이체 조정"]
```
...
> **해석**: 건별 분류가 78%여도 이체·카드 총액을 100% 잡으면 월 총지출 오차는 2% 미만이다. 완벽한 건별 분류 없이도 **저축률 추적**이 가능하다는 의미 — 완벽주의를 버리고 꾸준함을 선택하는 근거다.
...
**Q. 가계부를 쓰다가 자꾸 그만두게 되는데 어떻게 하면 유지할 수 있나요?**  
**A.** **핵심은:** 완벽한 기록보다 **오래 지속하는 것**이 훨씬 중요하다. 매월 1일에 카드명세서 합계, 이체 내역, 월급을 적는 세 줄만 써도 핵심 지표는 잡힌다. "80% 규칙"으로 전체 기록의 80%만 분류해도 충분히 의미 있는 분석이 가능하다.

**Q. ISA와 IRP 납입금은 가계부에 어떻게 기록하나요?**  
**A.** **T(투자·저축 이체)** 항목에 넣고, ISA/IRP 각각 메모로 구분한다. 이 금액은 생활비 예산에서 제외되어야 하므로, **월급 입금 후 가장 먼저 이체**하는 순서로 설계하면 지출 유혹을 줄일 수 있다.
