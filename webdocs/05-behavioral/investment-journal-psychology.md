...
!!! info "DCA (Dollar-Cost Averaging)"
    정기·정액 분할 매입.
...
**투자 일지를 왜 써야 하는가 — 한 줄 답:** 나 자신의 패턴을 발견하기 위해서입니다.

**구체적 작성 방법 (5분 버전):**
1. **날짜 + 자극**: "2024-03-15, 오후 11시, 커뮤니티에서 A 종목 +15% 소식 접함"
2. **감정 온도 (1~10)**: FOMO 강도를 숫자로 기록 — "7/10, 즉시 사고 싶다"
3. **실행 여부**: 매수/보류/무시 — "보류, DCA 날짜 아님"
4. **반증 시도**: "A 종목 반대 의견 1건 검색 — 부채비율 높음 확인"
5. **결과 (1주 후 기입)**: "+3% 추가 상승, 매수 안 한 것 후회 or 감사"

**핵심은:** 결과가 좋았는지 나빴는지보다, **과정이 규칙을 따랐는지**를 기록하는 것이 목적입니다. 규칙을 따른 보류가 손실을 냈어도 그것은 좋은 의사결정입니다.
...
flowchart TD
  trig[자극] --> filt{"IPS 통과"}
  filt -->|Yes| exe["허용 실행 DCA등"]
  filt -->|No| log["향동일지 패킷"]
  log --> cd["쿨다운 표준시각"]
  cd --> retr["후행 복기"]
  retr --> qa[패턴QA]
  qa --> rule{"규칙 개정여부"}
  rule -->|Yes| ips["IPS 소개정"]
  rule -->|No| keep[관찰유지]
  ips --> trig
  keep --> trig
```
...
```mermaid
flowchart TB
  h0["선행신념 theta0"] --> ev[증거관측]
  ev --> pplus["편향수집 경로Plus"]
  ev --> pminus{반증추구}
  pminus -->|예| upt["후행 theta1"]
  pminus -->|아니오| spiral[피드박확증]
  upt --> jr["판단품질지표 Q"]
  spiral --> jr2["판단품질하락 경고"]
```
...
**읽는 법**: **d**와 **W_**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **d**, **W_**, **t**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
...
**식 (기호)**: **P**(H|E)=(**P**(E|H) **P**(**H**)) / (**P**(**E**)) , **P**(**E**)=**P**(E|H)**P**(**H**)+**P**(E| **H**)**P**( **H**)
...
**읽는 법**: **P**와 **H**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **P**, **H**, **E**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
...
지연 \(T\) 시간) 의 **기대 후회 차**를 \( \Delta R = \mathbb{E}[ \text{regret}(\pi) - \text{regret}(\pi') ] \) 로 둘 때, **충동 지수**가 높은 기간일수록 \(\Delta R>0\) 가 되기 쉽다는 **정성命題**를 문장으로 기록합니다. 실제 수치화는 개인별 유틸리티 가정이 필요하므로 본 학습에서는 **등급**(낮음/중간/높음) 만 사용합니다 — 이는 과도한 과학적 위장 없이 학생이 **복잡도 경계 인식**을 갖도록 돕습니다.
