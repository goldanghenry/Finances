...
|   \(Cap rate\)   |   Cap rate   | \(Cap rate\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(NOI\) | Net Operating Income | 임대수익−운영비 |
|   \(가격\)   | \(가격\) | \(가격\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
\text{Cap rate} = \frac{\text{NOI}}{\text{가격}}
\]


**읽는 법**: **Cap**와 **NOI**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
