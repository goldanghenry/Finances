...
| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|             \(Cap\)             | Cap | Cap이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|             \(NOI\)             | NOI | NOI이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
**유도 (L4)**:
1. **정의**: **Cap**, **NOI**, **V**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
...
| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(\sum_j\)  |  sum j  | sum j이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(V_j\)  |  V j  | V j이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|            \(V\)            | V | V이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
**읽는 법**: **sum_j**와 **V_j**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **sum_j**, **V_j**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
단위증권(또는 주당) 귀속은
...
실무 논점: **건자산**은 시가·DCF·판매근거 불일치 가능| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(P\) | 포트 규모 | 가상 포트폴리오 규모(만 원) |
|            \(L\)            | L | L이(가) 이 식에서 맡는 역할(§4·본문 참고) |
, 장외 데이터라 **스튜던트 교육**과 **직접 거래 레벨**(아파트 시세 크롤) 직렬 비교 불가입니다.

### 6.3 “듀레이션 인튜이션”— 왜 채권 듀레이션을 그대로 못 포팅할까?

채권 **Macaulay / Modified Duration**은 고정coupon·만기 패턴 명확 증권에 정립되었습니다. 지분 리츠는 **가변 NOI·외생적 C| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(P\) | 포트 규모 | 가상 포트폴리오 규모(만 원) |
|            \(L\)            | L | L이(가) 이 식에서 맡는 역할(§4·본문 참고) |
ap rate 재평가**가 있어, 본 교육문서에서는 **회귀로 추정한 유효 민감도**(예: 무위험이자율 또는 텐저 변동에 대한 자산 회귀계수 근처) 또는 **금리변동 Scenario P\\&L**을 **두레이션 은유**로만 사용합니다:
...
**읽는 법**: **P**와 **L**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **P**, **L**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
