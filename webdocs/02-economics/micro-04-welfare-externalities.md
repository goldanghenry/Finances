...
> - "전기차 보조금이 왜 주어지는가? 그게 없어지면 배터리 기업 주가에 어떤 영향인가?"
> - "탄소세가 도입되면 화석연료 기업과 친환경 기업 중 어디가 유리한가?"
> - "반도체 보조금(미국 CHIPS Act, 한국 K-반도체)이 시장 경쟁을 왜곡하는가?"

**핵심은:** 외부성(externality)이란 내 행동이 제3자에게 미치는 영향입니다. 탄소 배출은 기업이 비용을 안 내지만 사회 전체가 피해를 보는 대표적인 부정적 외부성입니다. 정부가 보조금·세금으로 이를 "내부화"하려는 정책이 투자 환경을 직접 바꿉니다.
...
```mermaid
flowchart LR
  Neg[음의외부성] --> Gap["사적>사회 최적?"]
  Gap --> Pig["피구세 또는 규제"]
  Gap --> Coase["코스 협상"]
  Pos[양의외부성] --> Sub["보조금 R&D"]
  Trade["관세 쿼터"] --> Pdom[국내가격↑]
  Pdom --> DWL2["수입감소 DWL"]
```
...
```mermaid
flowchart TD
  Sub["EV 보조금 s"] --> Dshift["수요 우측"]
  Carb["탄소내부화 tau"] --> Sshift["공급 좌상"]
  Dshift --> Qup["단기 Q 판매↑"]
  Sshift --> Cost["셀 원가↑"]
  Qup --> Rev["매출 기대↑"]
  Cost --> Margin["마진 압박"]
  Expire["보조금 만료"] --> Qdown["수요 급감"]
  Capex["과거 CAPEX"] --> Oversupply["공급 과잉"]
  Qdown --> Cycle["사이클 하방"]
  Oversupply --> Cycle
```
...
### 7.4 2025~2026 한국 정책·투자 체크포인트 (외부성)

**쉽게 말하면:** 정부 보조금·규제가 내 투자에 미치는 영향입니다.

| 정책 | 미시 효과 | 투자 시사점 |
|------|------|------|
| 전기차 보조금 변화 | 수요곡선 이동 → EV 판매량 | 배터리·완성차 수요 민감도 |
| RE100·탄소배출권 | MSC 내부화 → 비용 상승 | 철강·시멘트·항공 마진 압력 |
| 반도체 특별법·세액공제 | 양의 외부성 지원 | CAPEX 증가 → 장비·소재 수혜 |
| CBAM(탄소국경조정) | 수출 비용 증가 | EU 수출 비중 높은 기업 영향 |
...
### 예제 3 — 단계별: 전기차 보조금 만료 시 수요 변화

**상황**: 전기차 정부 보조금 P = G, 수요 함수 Q = 100 - 2P, 공급 함수 P = 20 + Q

**보조금 있을 때:**
1. 구매자 실질 가격: 시장가 − G (예: G = 10)
2. 시장가 P = 20 + Q, 구매자가 보는 가격 P_d = P − G
3. 균형: 100 − 2(P − 10) = 2(P − 20) → 간단화하면 Q_1 > Q_0

**보조금 만료 시:**
- 수요량이 Q_0으로 감소
- 탄력성이 낮으면(비탄력적) 가격은 조금 오르고 수요 감소는 상대적으로 적음
- 탄력성이 높으면(탄력적) 수요 감소가 큼

**투자 시사점**: 보조금 만료 = 수요곡선 좌측 이동. 배터리 기업 판매량 감소 → 가동률·마진 압박. 이 효과의 크기는 "전기차 수요의 가격 탄력성"이 얼마인가에 달렸습니다.
...
**Q9. 탄소세가 도입되면 투자 포트폴리오는 어떻게 조정해야 하나?**  
**A9.** 쉽게 말하면: 탄소세는 화석연료 기업의 비용을 높이고, 친환경(태양광·풍력·배터리) 기업의 상대적 경쟁력을 높입니다. MSC(사회적 한계비용) > MPC(사적 한계비용)인 상황에서 세금이 그 차이를 메우는 것입니다. 탄소세 도입 국가·시장 노출이 많은 포트폴리오라면 ESG 관련 ETF 비중을 고려할 수 있습니다.

**Q10. 전기차 보조금이 없어지면 배터리 기업에 어떤 영향인가?**  
**A10.** 예를 들어: 보조금은 수요곡선을 인위적으로 오른쪽으로 밀어왔습니다. 보조금이 사라지면 수요가 줄어 판매량·가격이 모두 압력을 받습니다. 단, 배터리 가격이 자체적으로 충분히 내려가(비용 하락) 보조금 없이도 경쟁력이 생기면 이 효과가 상쇄됩니다. 핵심은 "보조금 제거 시점의 배터리 가격 경쟁력"입니다.
