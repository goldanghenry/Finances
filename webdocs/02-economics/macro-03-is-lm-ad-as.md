...
**비교정태**: \(c \uparrow \Rightarrow\) 승수 \(\uparrow\). \(\bar{C}, \bar{I}, G \uparrow \Rightarrow Y^* \uparrow\) 및 \(C=c(Y^*-T) \uparrow\).

### 6.2 IS 곡선 유도

투자을 **금리 함수**로: \(I = \bar{I} - b r\), \(b>0\).
...
**극단**: LM **수직**(고전) → \(G \uparrow\) 가 **전부 \(r\)** 로 흡수, \(Y\) **불변** — **완전 크라우딩아웃**.  
...
\(r\) 이 **최소 \(r_{\min}\)** (영% 근처)에서:

- **투기화폐 수요** 무한대 근사 → \(h \to \infty\) → LM **수평**: \(r = r_{\min}\)
