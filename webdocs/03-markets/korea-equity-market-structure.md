...
쉽게 말하면: 코스피200 ETF(KODEX200 등)를 사면 삼성전자·SK하이닉스·현대차 등 대형주에 **시가총액 비례로 투자**하는 것이다. 삼성전자 비중이 20~25%를 차지하므로, “코스피에 분산했다”고 해도 삼성전자 실적에 큰 영향을 받는다. 핵심은: 지수 ETF의 “분산”은 개별 종목 리스크를 줄이지만, **대형주·섹터 집중**은 여전히 존재한다.
...
실제 투자에서는 이렇게 씁니다: 코스닥 소형주는 하루 거래량이 수십만 주에 불과한 종목도 많다. 이런 종목을 시장가로 대량 매수하면 호가를 끌어올리며 체결되는 “충격 비용”이 발생한다. 반대로 매도할 때 원하는 가격에 못 팔 수 있다. 주의할 점: 코스닥 소형주는 분산이 어려워 개별 종목 리스크가 크다. 교육 프레임에서는 Bucket 4(위성) 소량으로만 다루고, 코어는 KOSPI200 ETF나 미국 지수 ETF로 구성하는 것을 권장한다.
...
```mermaid
flowchart TD
  Inv[투자자] --> Br["증권사 HTS MTS"]
  Br --> R{"주문 라우팅"}
  R -->|KRX| K["KRX 정규 장전 장후 등"]
  R -->|NXT| N[NXT ATS 세션]
  R -->|스마트| S["시장 자동선택"]
  K --> F["체결 예탁"]
  N --> F
...
**읽는 법**: **a**와 **b**의 차이를 **m**으로 나누면 **호가 스프레드** 근사가 된다. \(a\)는 최우선 매도호가, \(b\)는 최우선 매수호가다.


**유도 (L4)**:
1. **정의**: **a**, **b**, **m**을 동일 시점·동일 종목 호가로 맞춘다.
2. **식 변형**: 중간값 **m**으로 스프레드를 정규화한다.
3. **해석**: 스프레드가 넓으면 **체결 비용·유동성 리스크**가 커진다.

### 6.2 회전율 (일간)

| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(\mathrm{Turnover}_d\) | 일간 회전율 | 거래대금 대 시가총액 |
| \(\mathrm{거래대금}_d\) | 일 거래대금 | 당일 체결 금액 합 |
| \(\mathrm{시가총액}_d\) | 시가총액 | 유통 기준 규모 |

\[
\mathrm{Turnover}_d \approx \frac{\mathrm{거래대금}_d}{\mathrm{시가총액}_d}
\]

**식 (기호)**: **Turnover_d** ≈ (거래대금_d) / (시가총액_d)
...
**읽는 법**: **거래대금**을 **시가총액**으로 나누면 **유통 속도**를 본다. 


종목별 정의는 거래소·데이터 제공사 기준을 확인한다.

**유도 (L4)**:
1. **정의**: **거래대금**, **시가총액**의 집계 구간(일간)을 맞춘다.
2. **식 변형**: 분모·분자를 동일 통화·동일 유통주 기준으로 맞춘다.
3. **해석**: 회전율이 낮으면 **유동성·관심**이 약할 수 있다 — 섹터·시총 비교는 정의 통일 후.
