...
**핵심은:** 한국 증권사 앱에서 클릭 몇 번으로 미국 주식을 살 수 있는 시대입니다. 그러나 "쉽게 살 수 있다"와 "이해하고 관리할 수 있다"는 다릅니다. 환율·5월 양도세 신고·배당 원천징수·ISA 한도—이 4가지를 모르면 **수익이 났는데 세금 고지서가 오거나**, **ISA를 중도 해지해 비과세 혜택을 잃거나**, **환손실로 달러 수익이 원화에서 사라지는** 상황이 옵니다.
...
**쉽게 말하면:** 미국 주식 투자를 해외 직구에 비유합니다. 쇼핑몰(브로커)에서 상품(주식)을 달러로 삽니다. 배송(환전)에서 달러↔원화 환율이 개입합니다. 관세(세금)는 양도세(5월)와 배당 원천징수(미국 15% + 한국 정산)로 나뉩니다. 직구가 편해도 관세 신고를 잊으면 가산세가 붙듯, 해외주식도 **5월 신고를 잊으면 가산세**가 있습니다.

**핵심은:** 해외 직접 투자를 시작하기 전에 **딱 세 가지**만 확인합니다: (1) ISA 개설 여부, (2) W-8BEN 제출 여부, (3) 5월 신고 루틴 설정. 이 세 가지가 없으면 수익은 같아도 **세후 체감 수익률**이 달라집니다.
...
```mermaid
flowchart TD
  Buy["달러 매수 환전"] --> Hold[보유]
  Hold --> Div["배당 원천징수"]
  Hold --> Sell["매도 환전"]
  Sell --> Gain["양도차익 계산"]
  Gain --> Report["5월 종합소득 신고 등"]
  Div --> Report
```
...
### 7.5 국내 증권사 해외주식 계좌 개설 흐름 (교육)

**국내 주요 증권사 해외주식 매수 절차 (교육 — 가상 흐름)**:
1. **증권사 앱 설치** → 해외주식 메뉴 → 미국 주식 선택
2. **환전** → KRW → USD (환전 스프레드 확인, 우대 환율 이벤트 확인)
3. **W-8BEN 제출** → 앱 내 자동 처리 또는 서류 제출 (증권사별 상이)
4. **종목 검색** → QQQ, VOO, NVDA 등 → 시간대 확인 (미국 장은 한국 시간 야간)
5. **지정가 매수** → 달러 기준 → 체결 확인
6. **매도 후** → 양도 내역 저장 → 5월 신고 준비

**증권사별 해외주식 수수료 비교 (교육 — 가상 예시, 실제 확인 필요)**:

| 유형 | 수수료 범위 (교육) | 특징 |
|------|------|----------------|
| 대형 종합 증권사 | 0.25~0.35% | 서비스 풍부 |
| 온라인 특화 증권사 | 0.05~0.20% | 수수료 낮음 |
| 이벤트 할인 | 일정 기간 무료 | 기간 종료 후 원복 |

**쉽게 말하면:** 미국 주식 수수료는 증권사마다 다릅니다. 장기 보유 투자자는 수수료보다 **환전 스프레드**가 더 큰 비용이 될 수 있습니다. ISA 안에서 **국내 래핑 ETF**를 쓰면 환전 스프레드 없이 미국 지수에 투자할 수 있습니다.
...
- 매도 환율: **e₁**
- QQQ 수익률(달러): **R_USD**
- 양도세율(해외주 기준): **τ_cg** (본세 + 지방세, 교육 단순화)
- 기본공제: **D** (250만 원, 가상)

**단계 1 — 달러 주가 수익 계산**:

\[ 	ext{달러 이익} = M / e_0 	imes R_{USD} \]

**단계 2 — 원화 환산 이익 계산**:

\[ 	ext{원화 환산 이익} = 	ext{달러 이익} 	imes e_1 \]

**단계 3 — 취득·매도 원화 차익**:

\[ 	ext{양도차익} = 	ext{매도대금(원화)} - 	ext{취득대금(원화)} \]

**단계 4 — 세금 계산 (가상 단순화)**:

\[ 	ext{세금} = (	ext{양도차익} - D) 	imes 	au_{cg} \]

**단계 5 — 세후 수익률**:

\[ R_{	ext{세후}} = rac{	ext{원화 환산 이익} - 	ext{세금}}{M} \]

**교훈**: 
1. 환율이 **두 번**(매수·매도) 적용되므로 원화 가치가 변동 = 수익률에 직접 영향
2. 매도 타이밍을 12월 말 대신 **1월 초로 미루면** 공제 기간이 달라질 수 있음(규정 확인)
3. **ISA 안**에서 거래하면 이 계산 자체가 비과세 한도 내에서 단순화됨
...
**Q9. TIGER 미국나스닥100과 QQQ 직접 중 어느 것이 나은가요?**
**A.** 교육적 비교 기준: (1) **환헷지 여부** — TIGER는 환노출(H 없으면)/환헷지(H 붙으면) 선택 가능, QQQ는 항상 달러 노출. (2) **세금 경로** — TIGER는 국내 ETF 규칙, QQQ는 해외주 양도세+5월 신고. (3) **거래 시간** — TIGER는 한국 장 시간, QQQ는 미국 야간. (4) **TER** — TIGER가 높은 경우도, 낮은 경우도 있으니 확인. ISA 안에서는 두 가지 모두 가능하지만 세금 처리가 다를 수 있습니다.

**Q10. 해외주식 수수료는 얼마나 되나요?**
**A.** 증권사·이벤트·거래 유형마다 다릅니다. 교육 목적 범위: 온라인 특화 증권사는 미국 주식 기준 0.05~0.20% 수준, 대형 종합 증권사는 0.25~0.35% 수준이 있습니다(이벤트 기간 무료도 있음). **환전 스프레드(0.5~1.5% 수준)**도 중요 비용입니다. 실제 수수료는 해당 증권사 공시를 직접 확인하세요.
