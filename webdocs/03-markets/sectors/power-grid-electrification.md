...
**핵심은:** 전기 없이는 AI도 없습니다. "AI 데이터센터 투자 사이클"과 "전력망 병목"은 동전의 양면입니다. 2025~ 시장에서 "AI가 전기를 너무 많이 먹는다"는 이슈가 부각되면서, 전력망·변압기·ESS는 AI 투자 사이클의 **2차 수혜 섹터**로 분류됩니다. 한국은 수출용 **변압기·ESS** 분야에서 글로벌 경쟁력이 있어 특히 주목받고 있습니다.
...
**쉽게 말하면:** AI 데이터센터가 도시 하나만큼의 전기를 씁니다. 그 전기를 공급하는 데 **변압기 납기가 2~3년**인 경우가 있습니다. 즉, GPU를 주문해도 전기를 가져올 변압기가 없으면 데이터센터 증설이 막힙니다. 이것이 "전력망이 AI 투자의 병목"이라는 말의 의미입니다.

**핵심은:** 전력망·전기화 섹터의 투자 논리는 **"AI DC 증설 속도 × 전력 수요 증가 × 기존 그리드 노후화"** 세 가지 요인의 합력입니다. 유틸리티(전기 공급 회사)는 규제 요금으로 마진이 제한되고, 설비회사(변압기·케이블)는 CAPEX 사이클에 더 민감합니다. 둘을 같은 "전기화 섹터"로 뭉뚱그리면 안 됩니다.
...
```mermaid
flowchart LR
  DCplan["DC 100MW 계획"] --> Permit["전력 접속 허가"]
  Permit --> TransfoOrder["변압器 주문"]
  TransfoOrder --> Lead["리드타임 24-36mo"]
  Lead --> Delay["착공 지연"]
  Delay --> GPUidle["GPU 대기 비용"]
  GridUp["송전 증설"] --> Permit
```
...
```mermaid
flowchart TB
  Peak["저녁 피크"] --> EVchg["EV 급속충전"]
  Peak --> DR["수요반응 요금"]
  Night[야간] --> DCtrain["DC 학습부하"]
  ESS2[ESS] -->|방전| Peak
  ESS2 -->|충전| Night
  LFP["LFP 셀"] --> ESS2
```
...
변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유틸 ROE (규제)**:
...
| 영역 | 내용 | bucket |
|------|------|----------------|
| 변압기·GIS | AI DC·해외 수출 수주 호조 | ETF 또는 Bucket 4 |
| ESS | 재생에너지 연계·DC 백업 | [battery-lfp-ncm-ess.md](battery-lfp-ncm-ess.md) 연결 |
| 전력IT·SCADA | 스마트그리드 국내외 | 중견 IT |
| EV 충전 | 국내 보급 가속 | 정책 민감 |
| 유틸리티 | 한전 계열 | 요금 규제 |
...
| 항목 | 2025 | 2026 |
|------|------|----------------|
| RE100·RPS | 기업 가입 확대 | 의무화 범위 확대 논의 |
| AI DC 전력 | 수도권 집중 | 지방·해외 분산 논의 |
| 변압기 수출 | 미국·유럽 호조 | 생산능력 확대 |

### 7.3 한국 투자자 실전 가이드 (교육)

**전력망·전기화 ETF 투자 경로 (가상 예시)**:
1. **코어** (Bucket 3): 글로벌 클린에너지 ETF 또는 **XLU**(유틸리티) — ISA 내 소량
2. **위성** (Bucket 4): 국내 **변압기·전력IT·ESS** 관련 상장사 → 수주 공시 추적
3. 미국 직접: **GRID·ICLN** (청정에너지·그리드 ETF)

**한국 전력망 밸류체인 (교육)**:

| 세그먼트 | 한국 노출 | 특이사항 |
|------|------|----------------|
| 변압기 | 대형 전력설비사 | AI DC 수출 수주 호조 |
| 전력IT·SCADA | 중견 IT 기업 | 스마트그리드 |
| ESS | 배터리사 + ESS 전문사 | [battery-lfp-ncm-ess.md](battery-lfp-ncm-ess.md) |
| EV 충전 | 충전사업자 | 보급 속도 연동 |
| 유틸리티 | 한전 계열 등 | 요금 규제 민감 |

**핵심은:** 전력망 투자는 **수출향 설비사**(AI DC 수혜 직접)와 **국내 규제형 유틸리티**(요금 정책 종속)를 명확히 구분해야 합니다.

**법·정책**: 전기사업법·RPS 제도·국가전략기술 세제, [references/sources.md](../../references/sources.md)
...
### 예제 보강: 전력망 투자 수익 기호 계산 (가상)

**설정 (교육용 기호)**:
- 전력망 투자 금액: **I_grid** (달러)
- 규제 수익률(ROE): **r_reg** (규제 당국 승인)
- 운영 기간: **T** (년)
- 전력 수요 성장률: **g** (AI 데이터센터 등)
- 전기 요금: **P_elec** ($/MWh)

**전력 유틸리티 수익 모델 (단순화)**:

\[ R_{utility} = I_{grid} \times r_{reg} \]

규제 자산 모델 → **투자가 많을수록 이익 증가** (역설적)

**가상 AI 데이터센터 전력 수요 계산**:

\[ D_{DC} = N_{GPU} \times P_{GPU,kW} \times \text{PUE} \]

단, PUE = 전력 효율 계수 (1.2~1.5)

**투자 아이디어 연결**:

| 수혜 유형 | 투자 논리 | 변수 |
|------|------|----------------|
| 전력망 장비 | I_grid 확대 | g, r_reg 상승 |
| 원자력 발전 | P_elec 상승 | AI 수요 g |
| 전력 ETF | 포트 분산 | 규제 리스크 낮춤 |

**교훈**: 전력 섹터는 **AI CAPEX 수혜의 2차 파생** — GPU 과열 시에도 전력 수요는 꾸준히 증가
...
**Q11. 전력망 ETF는 AI ETF와 어떻게 다른가요?**
**A.** AI ETF는 소프트웨어·반도체·데이터센터 중심, 전력망 ETF는 유틸리티·변압기·ESS·케이블 중심입니다. **상관관계는 중간 정도**입니다. AI 투자 붐이 이어질수록 전력망 수요도 늘지만, 유틸리티는 규제·금리 변화에 더 민감합니다.

**Q12. 변압기 수주가 좋으면 무조건 주가가 오르나요?**
**A.** 수주는 **선행 지표**이지만 **마진·대금 회수·경쟁 심화**가 변수입니다. 수주가 폭증해도 원자재(규소강판·동) 가격이 오르면 마진이 줄 수 있습니다. 수주 발표 시 **매출 인식 시기·백로그 규모·마진 가이던스**를 같이 봐야 합니다.

**Q13. 전력·에너지 섹터에 ESG 규제는 어떤 영향을 미치나요?**
**A.** RE100·RPS·탄소중립 정책이 재생에너지·ESS 수요를 밀어줍니다. 한국 기업들이 RE100 달성을 위해 태양광·ESS 도입을 늘리면 국내 ESS 시장도 커집니다. 동시에 석탄·LNG 발전 설비 투자는 줄어드는 방향이어서, "전력망"이라는 큰 카테고리 안에서도 재생·ESS vs 화석 인프라를 구분해야 합니다.
