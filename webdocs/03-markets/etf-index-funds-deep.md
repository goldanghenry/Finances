...
핵심은: “패시브 ETF라 비용이 없다”는 오해다. 복제 방식(완전 복제 vs 샘플링 vs 합성), 증권대여 수입의 투자자 귀속 비율, 배당 원천징수 처리 방식에 따라 지수 수익률과 실제 ETF 수익률 사이에 **연 0.1~0.5%p 이상의 추적오차**가 발생한다.
...
실제 투자에서는 이렇게 씁니다: 국내 KODEX 미국나스닥100 ETF와 미국 직접 투자 QQQ는 같은 나스닥100 지수를 추종하지만, **환헤지 여부, 배당 처리, 보수 구조**가 다르다. 특히 환헤지 ETF는 환헤지 비용(달러 금리 - 원화 금리 차이)이 발생하므로 강달러 국면에서 유리하지만 장기로는 복리 비용이 누적된다.

**프리미엄/디스카운트 = 자판기 과금 차이**: 관리 가격표(NAV)·실제 카드 긁을 때 과금(**시세**)이 다르다면, 그 차이 중 구조적으로 설명 가능한 한도를 넘는 부분은 **경로 의존**이며 AP·시장 깊이 부족 또는 **패닉**에서 커진다.

주의할 점: 괴리율이 +1% 이상인 ETF를 시장가 매수하면 즉시 -1% 손실로 시작하는 것과 같다. 특히 거래량이 적은 채권·신흥국·섹터 ETF에서 자주 발생한다. 매수 전 **NAV 대비 괴리율을 증권사 HTS에서 확인**하는 습관이 필요하다.
...
```mermaid
flowchart LR
  Idx["지수 리벨런스"] --> PCF["구성증권 파일 PC"]
  PCF --> AP["참가적립 AP"]
  AP -->|"실물설정 Basket in"| ETF["ETF좌 CU"]
  AP -->|"환매 ETF좌반환"| BasketOut["종목바스켓 회수"]
  Inv["일반투자자 시장매매"] --> Mkt["KRX거래소 호가"]
  Mkt --> Dev["프리미엄 발생시"]
  Dev --> Arb["차익매매 유인"]
```
...
```mermaid
flowchart TD
  Gap["Prem Disc발생"] --> Cmp{"충분 깊이?"}
  Cmp -->|Yes| APtrade["AP 설정환매로 조정"]
  Cmp -->|No| Persist["괴리 지속"]
  Persist --> Risk["패닉 거래중단 FX등"]
```
...
```mermaid
flowchart LR
  Swp["TRS 카운터파티"] --> IdxRet[지수총수익근사]
  Col["담보 바스켓"] --> OTC["선도장외 증거금"]
  TRS --> Fees["헷지비용 CCY Basis"]
```
