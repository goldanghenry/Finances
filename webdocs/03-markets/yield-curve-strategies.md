...
| **선수** | [채권·고정수익 심화](bonds-fixed-income-deep.md) |
...
**핵심은:** 수익률 곡선을 공부하는 이유는 채권 포트폴리오를 직접 운용하기 위해서만이 아닙니다. 수익률 곡선의 형태는 **경기 전망과 중앙은행 정책**의 시장 집약판입니다. 역전 곡선(단기 > 장기)이 나타나면 시장은 "앞으로 금리가 내려갈 것"을 예상한다는 의미이고, 이는 경기 하강 시그널로 해석됩니다. 이를 알면 주식 코어 비중 조절의 타이밍 판단에도 활용됩니다.
...
**쉽게 말하면:** 수익률 곡선을 은행 예금 이자율 표에 비유합니다. 보통 **1개월 짜리 예금 금리 < 3년 짜리 금리** — 오래 맡길수록 금리가 높습니다(정상 곡선). 그런데 중앙은행이 단기 금리를 급격히 올리면 **단기 금리 > 장기 금리**가 됩니다(역전 곡선). 은행에 1개월 맡기는 것이 3년보다 금리가 높다는 것은 **"지금 단기는 비싸지만, 앞으로 금리가 내려갈 것"**이라는 시장 예상입니다.

**핵심은:** 수익률 곡선 전략(바벨·불렛·롤다운)은 기관 채권 운용자의 전문 영역이지만, **개인 투자자도 곡선 형태로 경기 싸이클 신호를 읽는 것**은 가능하고 유용합니다. "역전 = 무조건 경기침체"는 아니지만, 역전 구간이 지속되면 역사적으로 경기 하강 가능성이 높아졌습니다.
...
| 버터플라이·커브처 | 세 레그 무게·중간 과대·과소 | §4 | [glossary](../00-roadmap/glossary.md#버터플라이·커브처) |
| 바벌·불렛 | 만기 분포 양상 | §4 | [glossary](../00-roadmap/glossary.md#바벌·불렛) |
| 롤·캐리 | 시간·쿠폰·재투자 | §4 | [glossary](../00-roadmap/glossary.md#롤·캐리) |
| 키 레이트 | 레그별 금리 변동 민감도 | §4 | [glossary](../00-roadmap/glossary.md#키-레이트) |
...
### 예제 5: 수익률 곡선 전략 단계별 기호 계산 (가상)

**설정 (교육용 기호)**:
- 단기 채권 수익률: **y_s** (예: 2년물)
- 장기 채권 수익률: **y_l** (예: 10년물)
- 스프레드: **S = y_l - y_s**
- 채권 수정 듀레이션: **MD** (가격 민감도)
- 수익률 변화: **Δy**

**가격 변화 근사**:

\[ \Delta P \approx -MD \times \Delta y \times P_0 \]

**스티프닝 시나리오 계산 (가상)**:
- Δy_s = +15 bp, Δy_l = -5 bp → S 확대
- 단기 채권 손실: \(-MD_s \times 0.15\% \times P_0\)
- 장기 채권 이익: \(+MD_l \times 0.05\% \times P_0\)

**불렛 vs 바벨 비교 (가상 교육)**:

| 전략 | MD (가상) | Δy 가상 | 가격 변화 방향 |
|------|------|------|----------------|
| 불렛 (7년 집중) | MD₁ | 균등 스티프닝 | 중간 |
| 바벨 (2년+30년) | MD₁ (동일) | 비대칭 Δy | 상황 따라 ± |
| 쇼트 단기 + 롱 장기 | 차이 | 스티프닝 기대 | 이익 |

**교훈**:
1. 스프레드 **S**가 핵심 — 경기 신호이면서 포지션 손익도 결정
2. **MD × Δy**가 1%면 원금의 1% 손익 발생 — 채권도 "주식급" 변동 가능
3. 한국 투자자는 **미국 10년물 vs 2년물 스프레드** 변화를 환율·kospi 선행지표로 모니터링
...
**Q6. 수익률 곡선 역전이 반드시 경기침체를 의미하나요?**
**A.** 아닙니다. 역전은 경기침체의 **선행 신호**이지 **충분조건**이 아닙니다. 역전 이후 실제 침체 없이 정상화된 경우도 있습니다. 역전 기간이 길수록, 폭이 클수록 역사적으로 침체 확률이 높았지만, 이번이 과거와 다를 수 있습니다. 단일 지표로 투자 결정을 내리는 것은 위험합니다.

**Q7. 한국 개인 투자자에게 채권 ETF 투자가 의미 있나요?**
**A.** 포트폴리오 방어·재균형 용도로 의미 있습니다. 주식이 하락할 때 채권 ETF가 오르는 경향(역상관)이 있어 변동성을 낮춥니다. 단, 금리가 오르는 시기엔 장기 채권 ETF도 하락합니다. **만기가 짧은 단기 채권 ETF**(금리 민감도 낮음)로 현금 대안을 만들거나, **장기 채권 ETF**로 듀레이션 전략을 구현할 수 있습니다.

**Q8. 바벨 전략과 불렛 전략 중 언제 어떤 것이 유리한가요?**
**A.** 교육적 기준: **바벨**(단기+장기)은 단기 레그에서 유동성을 확보하면서 장기 레그에서 수익을 취합니다. 금리 곡선이 **평탄화(플랫닝)**될 때 유리할 수 있습니다. **불렛**(중만기 집중)은 중간 만기 금리가 하락할 때 유리합니다. 그러나 금리 변동 예측은 어렵고 두 전략의 차이가 소매 투자에서 유의미한 수준인지는 불분명합니다.
