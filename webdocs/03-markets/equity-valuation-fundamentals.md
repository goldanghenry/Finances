...
|   \(P_1\)   | P 1 | P 1이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
|  \(FCF_{n\)  |  FCF n  | \(FCF n\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(g_{term}\)  |  g term  | \(g term\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
**고성장** \(t=1..n\) 을 명시 예측, **터미널**은 **\(g_{term}\)** (보통 **명목 GDP| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(WACC\) | 가중평균자본비용 | 기업·프로젝트 할인율 근사 |
|            \(CAPM\)            | CAPM | CAPM이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(r_E\)  |  r E  | r E이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r_f\) | r f | 국채·예금 등 기준 금리 |
|  \(R_m\)  |  R m  | R m이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|            \(E\)            | E | E이(가) 이 식에서 맡는 역할(§4·본문 참고) |
·인플레** 이하)로 **Gordon** 형태. **FCFF** 는 **WACC**로, **FCFE** 는 **\(r_E\)** 로 할인.
