...
**쉽게 말하면:** DB는 “회사가 알아서 굴려주는 퇴직금 제도”이고, DC는 “내가 직접 굴리는 퇴직금 제도”입니다. **증권사 앱에서 퇴직연금 메뉴를 열었을 때 ETF 주문 버튼이 있으면 DC, 없으면 DB**라고 생각하면 됩니다.

**핵심은:** 퇴직연금 유형에 따라 할 수 있는 행동이 완전히 다릅니다. DB 가입자는 ISA·IRP를 별도로 설계해야 하고, DC 가입자는 70% 위험자산 한도를 지키며 직접 운용해야 합니다.

**예를 들어:** "회사 퇴직연금에 QQQ를 넣고 싶다"는 말은 DC에서만 가능합니다. DB에서는 불가능하고, 대신 **개인 IRP나 ISA**에서 해결해야 합니다.
...
```mermaid
flowchart TD
  Start["우리회사 퇴직연금"] --> Q1{"증권앱에서 본인이 ETF 주문?"}
  Q1 -->|예| DC["DC 가능성 높음"]
  Q1 -->|아니오| Q2{"복지·노무 확인"}
  Q2 -->|확정급여| DB[DB]
  Q2 -->|확정기여| DC
  DB --> ReadDB["db-pension + irp"]
...
```mermaid
flowchart TD
  Ten["10년 목표"] --> Type{"DB or DC"}
  Type -->|DB| Slot1["ISA IRP QQQ"]
  Type -->|DC| Slot2["DC ISA QQQ"]
  Slot1 --> Bucket["Bucket2b 3"]
  Slot2 --> Bucket
```
