...
**세제 혜택을 모르면 손해:** 예를 들어 해외 ETF에서 연간 수백만 원 이익이 발생하면, 일반 계좌에서는 기본공제(250만 원) 초과분에 22%를 납부해야 합니다. 반면 ISA 3년 유지 후 비과세 한도(일반형 200만 원, 서민형 400만 원) 이내는 세금 0원, 초과분도 9.9%만 냅니다. 같은 투자를 해도 **연간 수십만~수백만 원**의 세금 차이가 생길 수 있습니다. ISA 납입 한도는 **연 2,000만 원**(2025 기준)으로, 매달 꾸준히 적립하면 3년 동안 최대 6,000만 원을 세제 우대 통장에 넣을 수 있습니다.
...
**쉽게 말하면:** ISA는 “**세금 절약 특별 계좌**”입니다. 3년 규칙만 지키면 계좌 안 수익에 정상 세율(해외주식 22%)보다 훨씬 낮은 세금(비과세 한도 내 0%, 초과분 9.9%)을 냅니다.

**핵심은:** 같은 QQQ ETF를 사더라도 **어느 계좌에 넣느냐**에 따라 세후 수익이 달라집니다. ISA 안에서는 손실 종목과 이익 종목을 **통산**해 세금을 줄일 수 있고, 비과세 한도까지는 아예 세금이 없습니다.

**예를 들어:** 3년 ISA 안 해외 ETF 순이익이 비과세 한도 200만 원(2025 일반형 ISA 기준) 이내이면 세금 0원, 초과분만 9.9%입니다. 일반 계좌에서 같은 해외 ETF를 팔면 **매년 5월 양도세 신고** + 기본공제(연 250만 원) 초과분에 22%가 붙습니다. 장기 DCA라면 세금 차이가 수십만~수백만 원에 달할 수 있습니다.
...
```mermaid
flowchart TD
  DB["DB 재직 ETF불가"] --> Q{"3년 유동성 확보?"}
  Q -->|예| ISA["ISA 코어 QQQ"]
  Q -->|아니오| IRP["IRP 추가납입 이연"]
  ISA --> Gen["일반 위성 소액"]
  IRP --> Gen
```
...
|------|------|----------------|
| QQQ 매매차익(3년 누적) | **M** (만 원 단위, 교육용) | **M** (만 원 단위, 교육용) |
| 비과세 한도(2025) | 200만 | — |
| 초과 **M** | 9.9% ≈ **M** | 22% 공제 후 ≈ **M** (단순) |
...
**Q11. 직장인이면 ISA와 IRP 중 무엇을 먼저 가입해야 하나?**  
**A11.** 일반적으로 **유동성이 높은 ISA 먼저** 권장합니다. ISA는 3년 후 만기 해지·재가입이 가능하고 자금 일부를 돌릴 수 있습니다. IRP는 55세 이전 인출 시 불이익이 크므로 비상금·중기 목돈이 준비된 뒤 IRP를 채우는 순서가 일반적입니다. 단, **세액공제 혜택(IRP 연 900만 원 한도)** 이 크다면 ISA와 IRP를 병행 검토합니다. 개인 상황에 따라 [isa-irp-practical-setup.md](isa-irp-practical-setup.md) 부록 A의 우선순위를 참고하세요.

**Q12. DB 연금이 있는데 ISA에 얼마나 넣어야 하나?**  
**A12.** DB 재직 중에는 ETF 직접 투자가 ISA·IRP에서만 가능합니다. ISA 납입 한도(연 2,000만 원)를 기준으로, 월 납입 가능 금액에 맞춰 자동이체를 설정하는 것이 핵심입니다. [isa-irp-practical-setup.md](isa-irp-practical-setup.md)의 체크리스트를 따르면 실수를 줄일 수 있습니다.
