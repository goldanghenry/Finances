...
**쉽게 말하면:** 청년미래적금은 청년도약의 “업그레이드판 정책 적금”입니다. 만기가 5년 → 3년으로 짧아지고, 금리·정부기여 구조가 바뀝니다. 보도상 3년 유지 시 정부 보너스 최대 약 216만 원 수준이 언급됩니다(개인별 상이, kinfa 확인).

**핵심은:** 갈아타기 순서가 틀리면 정부기여금을 잃을 수 있습니다. 반드시 ① 미래적금 가입신청 → ② 가입대상 통보 → ③ 계좌개설 → ④ 도약 특별중도해지 순서를 지켜야 합니다. 도약 먼저 해지하면 갈아타기 불가합니다.

**예를 들어:** 도약 가입자가 2년 경과 후 6월에 갈아타기를 하면, 기존 기여금은 특별중도해지로 보전되고 미래적금 3년을 새로 시작합니다. 반면 도약을 일반 해지하면 이미 받은 정부기여금도 반환해야 할 수 있습니다.
...
```mermaid
flowchart TD
  A["1 미래적금 가입신청"] --> B["2 가입대상 통보"]
  B --> C["3 미래적금 계좌개설 납입제한"]
  C --> D["4 도약 특별중도해지"]
  D --> E["5 미래적금 납입개시"]
  X["도약 일반해지 먼저"] -.->|"갈아타기 불가"| Fail[실패]
```
...
**식 (기호)**: 만기 수령액 ≈ Σ_t=1}^T **P_t** ·(1 + **r_eff**,**t**) + **G_t**
