...
**쉽게 말하면:** QQQ ETF의 배당은 “**운용사가 분기마다 보내주는 현금**”입니다. 이 배당을 받으면 금융소득에 합산되어 **이자·다른 배당과 함께 연 2,000만 원 기준**으로 과세됩니다.

**핵심은:** 해외주식 배당과 양도차익은 세금 계산에서 완전히 분리됩니다.
- **양도차익**: 매도 시 발생 → 양도소득세(22%), 5월 신고
- **배당소득**: 받을 때 발생 → 금융소득 합산 → 2,000만 원 초과 시 종합과세

**주의:** 미국 ETF(QQQ 등) 배당은 **미국에서 먼저 15% 원천징수**됩니다. 이 금액은 외국납부세액공제로 한국 세금에서 차감할 수 있습니다. 단, 2025년부터 DC·IRP 계좌의 해외 ETF 배당 **선환급 제도가 폐지**되어 실효 수익이 달라졌습니다.
...
```mermaid
flowchart TD
  Div["해외 배당 수령"] --> WH["현지 원천징수 약15%"]
  WH --> KR["국내 14% 차감 등"]
  KR --> Sum{"연간 금융소득"}
  Sum -->|"2000만 이하"| End["분리과세 종료 가능"]
  Sum -->|초과| May["5월 종합과세"]
  May --> FTC[외국납부세액공제]
```
