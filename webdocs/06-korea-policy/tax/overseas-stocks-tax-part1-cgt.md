...
```mermaid
flowchart TD
  Sell[해외주식_매도] --> Gain[양도차익_계산]
  Gain --> Ded[연250만원_공제]
  Ded -->|잔여_있음| Tax[22%_분리과세]
  Ded -->|0이하| Zero[양도세_0]
  Tax --> May[익년_5월_신고납부]
```
...
|  \(양도차익\)  | \(양도차익\) | \(양도차익\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(양도가액\)  | \(양도가액\) | \(양도가액\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(취득가액\)  | \(취득가액\) | \(취득가액\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|   \(필요경비\)   | \(필요경비\) | \(필요경비\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
\text{양도차익} = \text{양도가액} - \text{취득가액} - \text{필요경비}
\]


**읽는 법**: **양도차익**와 **양도가액**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
