...
**쉽게 말하면:** ISA·IRP·연금저축·DC는 각자 다른 세금 구조를 가진 계좌입니다. 같은 ETF라도 어느 계좌에 넣느냐에 따라 세금이 달라집니다.

**핵심 비교 (단계별 요약):**
- **ISA**: 납입 시 세금 없음 → 운용 중 비과세(통산) → 3년 만기 후 비과세 한도(200만/400만 원) + 초과분 9.9%
- **IRP·연금저축**: 납입 시 세액공제(합산 연 900만 원 한도, 공제율 13.2~16.5%) → 운용 중 과세이연 → 수령 시 연금소득세
- **DC**: 회사 기여금 + 가입자 추가납입(2026 +300만 DC만) → 과세이연 → 수령 시 과세
- **일반 계좌**: 즉시 과세(해외주식 매매차익 22%, 배당 금융소득 합산)

**주의:** ISA의 비과세는 "납입 원금"이 아니라 3년 누적 **순이익**에 적용됩니다. ISA에 2,000만 원을 넣어도 손실이 나면 비과세 혜택을 받을 이익 자체가 없습니다.
...
```mermaid
flowchart LR
  Pay["연간 현금여력"] --> ISAq{"3년 묶을 수 있나"}
  ISAq -->|예| ISA["ISA 납입한도"]
  ISAq -->|아니오| IRP["IRP 900만 공제"]
  ISA --> IRP
```
