...
**쉽게 말하면:** 같은 QQQ를 어디에서 사느냐에 따라 세금이 다릅니다. 이 문서는 “나는 어느 서랍을 써야 하나?”를 결정하는 **의사결정 지도**입니다.

| 계좌 | QQQ 매매차익 세금 | QQQ 배당 세금 | 비고 |
|------|------|------|------|
| 일반 계좌 | 22%(기본공제 250만 원/년 후) | 금융소득 합산 | 매년 5월 신고 |
| ISA 3년 | 비과세 한도 내 0%, 초과 9.9% | ISA 내 통산 | 중도해지 주의 |
| IRP/DC | 과세이연(수령 시 연금소득세) | 과세이연 | 2025~ 해외 배당 선환급 폐지 |

**주의:** 한 가지 계좌에 모든 것을 넣으면 안 됩니다. ISA는 연 납입 한도(2,000만 원)가 있고, IRP는 유동성이 낮습니다. 일반 계좌는 세금이 많지만 자유롭습니다. 본인의 목표에 맞게 역할을 나눠야 합니다.
...
**식 (기호)**: _eff = (**T_ISA** + **T_IRP** + **T_gen**) / (**G_total**)
...
**읽는 법**: **tau**와 **eff**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
---
...
ARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
---
...
```mermaid
flowchart TD
  Hold["해외 ETF 보유"] --> Sell{매도?}
  Sell -->|예| P1["Part1 양도세 5월"]
  Sell -->|아니오| Div["배당 수령"]
  Div --> P2["Part2 금융소득"]
  P1 --> Acct{계좌}
  Acct -->|ISA| P3["Part3 통산"]
  Acct -->|IRP| P3
  Acct -->|일반| P1
```
