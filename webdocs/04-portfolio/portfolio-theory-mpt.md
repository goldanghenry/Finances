...
| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(w_A\)  |  w A  | w A이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(w_B\)  |  w B  | w B이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
**직관**: \(ρ_{AB}=1\) 이면 \(σ_p = w_A σ_A + w_B σ_B\) (가중합, 분산 이득 없음). \(ρ_{AB}<1\) 이면 제곱근 안의 | 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |

교차항이 **감소**하여 \(σ_p\) 가 **가중합보다 작을 수** 있다. \(ρ_{AB}=-1\) 이고 적절한 \(w\) 면 이론상 \(σ_p \to 0\) 도 가능(교육용 극단).

**최소분산 비중** (2자산, 교육):

\[
w_A^* = \frac{σ_B^2 - ρ_{AB} σ_A σ_B}{σ_A^2 + σ_B^2 - 2ρ_{AB} σ_A σ_B}
\]
...
\[
μ_p = \mathbf{w}'\boldsymbol{μ}, \quad σ_p^2 = \mathbf{w}'\boldsymbol{Σ}\mathbf{w}
\]
...
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
\(\boldsymbol{Σ}\) 는 **공분산 행렬**. MPT의 계산 부담은 **추정 오차**(\(μ, Σ\))에 있다 — L4에서 반드시 “모형 vs 실무”를 구분한다.

### 6.4 효율적 투자선

**제약**: \(\sum w_i = 1\) (순수 위험자산 포트), 각 \(w_i\) 허용 범위(공매도 금지 시 \(w_i \ge 0\)).

**목표** (예): \(σ_p^2\) 최소화 subject to \(μ_p = \bar{μ}\) → | 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
|           \(S\)           | S | 소득 대비 남는 비율 |
Lagrange → **EF** 한 가지 \(μ\) 당 하나의 **최소분산 포트**.

**지배**: 포트 X 가 Y 보다 \(μ_X \ge μ_Y\) 이고 \(σ_X \le σ_Y\) 이고 적어도 하나는 엄격 → Y 는 비효율.
...
**기하학적 직관**: μ–σ 평면에서 점 \((σ_p, μ_p)\) 와 \((0, R_f)| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(S_p\)  |  S p  | S p이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(R_f\) | 무위험금리 | 국채·예금 등 기준 금리 |
|  \(S_M\)  |  S M  | S M이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
| A 주식 | 8% | 18% |
| B 채권 | 4% | 6% |
|   ρ_AB   |   0.2   | ρ AB이(가) 이 식에서 맡는 역할(§4·본문 참고) |
**50:50** (\(w_A=0.5\)):
