...
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(S_p\)  |  S p  | S p이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(R_f}\) | 무위험금리 | 국채·예금 등 기준 금리 |
|  \(gma_p}\)  |  gma p  | \(gma p\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|           \(R\)           | R | 기간당 이자·요구수익률 |
 **초과** — **표본·선택편향** 주의.
...
| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(S_{annual}\)  |  S annual  | \(S annual\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r_{f\) | r f | 국채·예금 등 기준 금리 |
|  \(gma_m}\)  |  gma m  | \(gma m\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
**읽는 법**: **S_**와 **r_**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **S_**, **r_**, **gma_m**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
...
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(R_f\) | 무위험금리 | 국채·예금 등 기준 금리 |
|  \(gma_d\)  |  gma d  | gma d이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(\sum_{t}\)  |  \sum t  | \(\sum t\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(R_{p\)  |  R p  | \(R p\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|           \(R\)           | R | 기간당 이자·요구수익률 |
| \(T\) | 기간 | 마지막 CF 시점 |
...
**읽는 법**: **sigma_d**와 **MAR**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **sigma_d**, **MAR**, **R_**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(R_f}\) | 무위험금리 | 국채·예금 등 기준 금리 |
|  \(eta_p}\)  |  eta p  | \(eta p\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|           \(R\)           | R | 기간당 이자·요구수익률 |
\[
T_| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
|            \(CAPM\)            | CAPM | CAPM이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(r_{a\)  |  r a  | \(r a\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(r_{p\)  |  r p  | \(r p\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(r_{b\)  |  r b  | \(r b\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(r_a\)  |  r a  | r a이(가) 이 식에서 맡는 역할(§4·본문 참고) |
