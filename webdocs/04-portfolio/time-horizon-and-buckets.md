...
**한국 투자자의 실수 사례 — 목적 혼동.** 2021년 코스닥 급등기에 "3년 후 결혼 자금"을 코스닥 단타 위주로 굴리다가 2022년 하락에 원금의 40%를 잃은 사례가 많았습니다. **핵심은:** 결혼 자금(2~3년 Bucket 1)은 주식이 아니라 예금·단기채에 있어야 합니다. 시간 지평이 목적을 결정하고, 목적이 자산군을 결정합니다.

**단기 자금 vs 단타**도 구분해야 합니다. 2년 뒤 전세 보증금은 **단기 자금**(목적이 확정, 원금 보존)이지, **단타**(매매 빈도)가 아닙니다. 반대로 10년 뒤 은퇴 자금을 **일간 매매**로 굴리는 것은 **기간은 장기인데 행동은 단타**인 모순입니다. **당신의 포트폴리오**에서 각 자금에 버킷 번호를 붙여보세요 — Bucket 0~1에 주식이 들어 있다면 즉시 재배치를 검토해야 합니다. Bucket 모델은 이 두 축을 표로 분리합니다.
...
flowchart TB
  Income["월급 보너스 기타소득"]
  Income --> B0["Bucket0 비상금 3~6개월"]
  Income --> B1["Bucket1 청년도약 미래적금"]
  Income --> B2a["Bucket2a DB 회사적립"]
  Income --> B2b["Bucket2b ISA IRP DC"]
  B2b --> B3["Bucket3 코어 ETF"]
  B2b --> B4["Bucket4 위성 단타"]
  B3 --> Goal["10년plus 은퇴 자산목표"]
  B4 --> Exp["학습 테마 NXT장후"]
```
