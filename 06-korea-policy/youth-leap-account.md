...
**쉽게 말하면:** 청년도약계좌는 “**정부가 월 최대 3만 원 넘게 보너스를 주고, 이자도 비과세인 5년 만기 적금**”입니다. ETF처럼 원금이 오르내리지 않고, 정해진 금리와 정부기여금이 쌓입니다.

**핵심은:** 청년도약을 주식 대신 쓰면 안 됩니다. 이 계좌는 **Bucket 1(고정·안정)**이고, 주식 투자는 **ISA·IRP(Bucket 2b~3)**에서 따로 해야 합니다. 둘을 병행하면 안정(도약)과 성장(ISA ETF)을 동시에 설계할 수 있습니다.

**예를 들어:** 월 40만 원씩 5년(60개월) 납입 시, 원금은 총 2,400만 원 + 정부기여금(소득·요건별, 보도상 월 최대 3.3만 원 수준) + 이자 비과세 = 실수령액이 원금보다 상당히 많아질 수 있습니다. 단, 중간에 일반 중도해지하면 정부기여금을 반환해야 합니다.
...
```mermaid
flowchart TD
  Elig["가입요건 kinfa"] --> Open[계좌개설]
  Open --> Pay["월 1천~70만 60개월"]
  Pay --> Gov["정부기여 매월"]
  Pay --> Int["이자 비과세"]
  Pay -->|"일반 중도해지"| Loss["기여 세제 상실"]
  Pay -->|"만기 또는 특별중도해지"| OK["혜택 유지"]
```
...
```mermaid
flowchart LR
  Y2025["2025 신규가능"] --> End["12.31 일몰"]
  End --> Y2026["2026 신규불가"]
  Y2026 --> Exist["기존자 5년유지"]
  Y2026 --> Switch["6월 미래적금 갈아타기"]
```
...
```mermaid
flowchart TD
  Leap["청년도약 가입중"] --> Due{"만기 또는 6월?"}
  Due -->|"6월 갈아타기"| Read["youth-future-savings 읽기"]
  Read --> Kinfa["kinfa 신청"]
  Due -->|만기| Mature["만기금 배분"]
  Mature --> ISA["ISA IRP 이전 검토"]
```
