...
**쉽게 말하면:** 연금저축은 “**노후를 위해 매달 스스로 채우는 세금 혜택 저축통**”입니다. IRP는 “**퇴직금도 들어오고, 추가 납입도 되는 더 큰 저축통**”입니다. 두 통 합쳐서 연 900만 원(2025 기준)까지 납입하면 **세액공제**를 받습니다.

**핵심은:** 연금저축과 IRP는 둘 다 좋지만, 세액공제 한도를 **합산해서 사용**합니다. 900만 원 한도를 두 계좌에 나눠 넣든, 한 계좌에 몰아넣든 공제 금액은 동일합니다. 단, IRP는 퇴직금 이전이 가능하고 연금저축은 불가하다는 차이가 있습니다.

**예를 들어:** 세액공제율 13.2% 구간(총급여 5,500만 원 초과)이면, 연금저축+IRP 합산 900만 원 납입 시 절세액은 900만 × 13.2% = **약 118.8만 원**입니다. 16.5% 구간(총급여 5,500만 원 이하)이면 900만 × 16.5% = **약 148.5만 원**이 됩니다. 이 금액이 매년 환급되고 다시 투자되면 장기 복리 효과가 상당합니다.
...
flowchart LR
  ...
  DB -.->|"재직중 QQQ직접X"| Limit[운용제한]
  IRP2 --> ETF["QQQ등 ETF가능성"]
  DC --> ETFdc["DC안 ETF선택가능성"]
  ISA --> ETFisa[비연금세제]
```
...
**읽는 법**: **절세**와 **근사**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **절세**, **근사**, **적격 납입액**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
...
**읽는 법**: **FV**와 **sum_**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **FV**, **sum_**, **P_t**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
...
**식 (기호)**: **FV_net** ≈ Σ_t=1}^n **P_t** (1+**r**-)^ **n**-**t**
...
**읽는 법**: **FV_**와 **sum_**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **FV_**, **sum_**, **P_t**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
...
---
 ...
 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **FV_**, **sum_**, **P_t**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
...
같은 **M** 납입이라도 ETF A(미국 장기 채권) vs ETF B(글로벌 주식)는 **변동성·배당·환율**이 다릅니다. 교육용으로 B의 연 변동성이 A의 두 배이라 하면 **정신적 계좌(행동 편향)** 차원에서 B는 다음 해 납입을 건너뛸 확률이 높다고 실험에서 자주 관찰됩니다. 그래서 L4에서는 **목표 비중을 먼저** 정하고 분할 매수합니다.
