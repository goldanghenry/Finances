...
**쉽게 말하면:** IRP는 “**노후 대비 세금 우대 계좌**”입니다. 지금 연금저축+IRP 합산 **연 900만 원**까지 납입하면 납입액의 **13.2%~16.5%** 를 연말정산에서 세액공제 받을 수 있습니다. 예를 들어 900만 원 납입 시 최대 148.5만 원의 세금이 줄어듭니다.

**핵심은:** IRP는 세 가지 기능이 합쳐진 계좌입니다. ① 재직 중 **ETF 직접 투자**(DC처럼), ② 납입 시 **세액공제**, ③ 퇴사 후 **퇴직금 이전** 통로. DB 가입자는 ①②가 특히 중요합니다.

**예를 들어:** 연봉 5,500만 원 이하 직장인이 IRP에 연 900만 원을 납입하면, 세액공제율 16.5% 적용 시 연말정산에서 **IRP 세액공제 한도 범위 내 약 148만 원**을 돌려받을 수 있습니다. 매년 이 금액이 복리로 재투자된다면 장기적으로 상당한 차이가 됩니다.
...
flowchart TB
  subgraph work [재직중]
    DB["DB 회사적립 본인매매없음"]
    IRP["IRP 개인추가납입 ETF선택"]
  end
  subgraph leave [퇴사]
    Choice{"일시금 vs IRP이전"}
    Choice -->|IRP| IRP2["IRP 퇴직금+운용권"]
    Choice -->|일시금| Cash["퇴직소득세 후 현금"]
  end
  DB --> Choice
  IRP --> IRP2
```
...
**Q9. DB 연금이 있는데 IRP에 추가로 넣어야 하나?**  
**A9.** 네, 많은 경우 **개설 가치가 큽니다.** DB는 회사가 운용하므로 본인이 QQQ 같은 ETF를 살 수 없습니다. IRP를 열면 ① 본인이 직접 ETF를 고를 수 있고, ② 납입 시 **세액공제(IRP 한도 포함 연 900만 원)**를 받을 수 있습니다. 단, 유동성이 낮으므로 비상금 확보 후 납입하는 것이 원칙입니다.

**Q10. IRP와 연금저축을 둘 다 열면 세액공제 한도가 두 배가 되나요?**  
**A10.** **아닙니다.** IRP + 연금저축의 세액공제 한도는 합산하여 연 900만 원(2025 기준)입니다. 두 계좌를 병행해도 같은 바구니를 나눠 쓰는 것입니다. 어느 계좌에 얼마나 넣을지는 수수료·상품 목록을 비교해 결정하세요.
