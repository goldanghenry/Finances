...
|------|------|------|----------------|
| 금융소득 | 이자소득·배당소득 등 **현금 또는 현금등가 정기 지급** 위주 과세 카테고리 | §4 | [glossary](../../00-roadmap/glossary.md#금융소득) |
| 분리 과세 종결 가능성 | 특정 신고 패키지로 **종합 과세 안으로 반드시 들어가지 않고** 간소하게 정리 가능한 상태 | §4 | [glossary](../../00-roadmap/glossary.md#분리-과세-종결-가능성) |
| 종합 과세·종합합산 | 과세표준 산출 시 다른 소득과 **통합 과세 프로그램**에서 합산하는 구조적 가능성 | §4 | [glossary](../../00-roadmap/glossary.md#종합-과세·종합합산) |
| 원천징수 | 지급 시점 과세 패키지 | §4 | [glossary](../../00-roadmap/glossary.md#원천징수) |
| 과세 게이트(교육) | 원칙 2천만 원 초과 시 **설계 패러다임 전환** | §4 | [glossary](../../00-roadmap/glossary.md#과세-게이트) |
| 금융투자소득세 | 미디어에서는 **별도 과세 프로그램**으로 보도되는 체계 | §4 | [glossary](../../00-roadmap/glossary.md#금융투자소득세) |
| 증권 양도소득(해외) | 브로커 증평차익 과세 프로그램 | §4 | [glossary](../../00-roadmap/glossary.md#증권-양도소득) |
| ISA 손익통산·한도 비과세 | ISA **내부** 중도 운영 시 **패키지**가 다른 **계좌 수준 과세 프로그램** | §4 | [glossary](../../00-roadmap/glossary.md#isa-손익통산·한도-비과세) |
| 과세표준 증분(한계) | 종합 과세 프로그램에서 **소득 1단위 증분**당 **실효 추가 세금** 패키지 | §4 | [glossary](../../00-roadmap/glossary.md#과세표준-증분) |
...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| **F** | 금융소득 합 | 교육용 누적 휴리스틱(법령 제외·포함 별도) |
| \(F^{\ast}\) | 종합과세 문턱 | 통상 2,000만 원(교육) |
| \(I_{\text{dom}}\) | 국내 이자 | 국내 원천 이자소득 |
...
**식 (기호)**: **F** = **I_dom** + **I_for** + **D_dom** + **D_for** + ·**s**


**식 (기호)**: **F** = **I_dom** + **I_for** + **D_dom** + **D_for** + ·**s**


**읽는 법**: **F**는 항목 **합**이지만, 실무에서는 **포함·제외**를 법령·신고 프로그램으로 확정한다.


**유도 (L4)**:
1. **정의**: **F** 항목을 국내·해외·이자·배당·양도로 분리해 기록한다.
2. **식 변형**: 연간 합산 후 **F***와 비교한다.
3. **해석**: 항목 누락이 **문턱 판단**을 바꿀 수 있다.
...
**식 (기호)**: **F** F^ (통상 F^=20{,}000{,}000~원)


**식 (기호)**: **F** F^ (통상 F^=20{,}000{,}000~원)


**읽는 법**: **F**가 **F*** 이하면 **분리과세** 패키지, 초과 시 **종합과세** 설계를 검토한다.


**유도 (L4)**:
1. **정의**: **F**와 **F***를 동일 과세연도·동일 통화로 맞춘다.
2. **식 변형**: 초과분만 별도 시나리오로 분리한다.
3. **해석**: 문턱 직전·직후의 **한계 세부담**이 설계를 좌우한다.
...
**식 (기호)**: (**d** **T_agg**) / (**d** **F**)|_(개인 상태) (개념)


**식 (기호)**: (**d** **T_agg**) / (**d** **F**)|_(개인 상태) (개념)


**읽는 법**: 근로소득 구간·세액공제 교착 때문에 **증분 세율**이 선형이 아닐 수 있다.


**유도 (L4)**:
1. **정의**: **T_agg**를 신고 프로그램 출력과 대조한다.
2. **식 변형**: **F**를 소폭 변경해 **증분**을 수치·개념으로 본다.
3. **해석**: 선형 가정이 깨지면 **계단형** 세부담을 전제로 설계한다.
...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| **G** | ISA 내부 이익 | 계좌 내 누적 손익(교육) |
| **H** | 비과세 한도 | 제도상 비과세 구간 |
| \(S_{\text{ISA}}\) | ISA 우위(개념) | 일반계좌 대비 세후 이익 근사 |

\[
S_{\text{ISA}} \approx \text{(일반 해외 양도·배당 혼합 대비)} \quad (\text{단순})
\]

**식 (기호)**: **S_ISA** ≈ (일반 해외 양도·배당 혼합 대비) (단순)


**식 (기호)**: **S_ISA** ≈ (일반 해외 양도·배당 혼합 대비) (단순)


**읽는 법**: **ISA**는 **G**·**H**와 **종합과세**·**해외 증빙**이 맞물려 **항상 단순 대체**되지 않는다 — [isa-irp-pension-tax.md](isa-irp-pension-tax.md).


**유도 (L4)**:
1. **정의**: **G**, **H**를 ISA 계좌 스펙·비과세 구간으로 맞춘다.
2. **식 변형**: 일반계좌 시나리오와 **세후**를 나란히 둔다.
3. **해석**: 우위가 **항상** 성립하지 않음을 전제로 분기별 점검한다.
...
| 항목 | 금액(가상) |
|------|------------|
| 국내 은행 이자 | **M** (만 원 단위, 교육용) |
| 해외 ETF 배당(원화환산) | 1,**M** (만 원 단위, 교육용) |
| **합계 \(F\)** | ****M** (만 원 단위, 교육용)** → **게이트 초과(원칙)** |
...
| 항목 | 금액(가상) |
|------|------------|
| 해외 양도 이익 | **M** (만 원 단위, 교육용) |
| 양도세 프로그램(단순) | 공제 후 **별도 축** |
| 배당·이자 합 \(F\) | 1,**M** (만 원 단위, 교육용) → **게이트 이하(원칙)** |
