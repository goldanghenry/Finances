...
|  SAFE  | SAFE | SAFE이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
| \(Post-money\) | 포스트머니 | **투자** **후** **기업가치** |
| \(Pre-money\) | 프리머니 | **투자** **전** **기업가치** |
|             \(Investment\)             | Investment | Investment이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
**유도 (L4)**:
1. **정의**: **Post**, **Pre**, **Investment**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
...
\[
\text{Founder| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
| \(n\) | 기간 | 연·월 등 복리·할인에 쓰는 횟수 |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |

 \% after} = \frac{\text{Founder shares}}{\text{Total fully diluted shares after round}}
\]
...
| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(EV_{\text{exit}}\)  |  EV \textexit  | \(EV \textexit\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(T\) | 기간 | 마지막 CF 시점 |
...
**읽는 법**: **EV_**와 **T**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **EV_**, **T**, **M**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.

\(EV_{\text{exit}}\): **Exit** **가치**, \(r\): **할인율**, \(T\): **연수**, \(M\): **목표** **배수** (예: **10x**).

**가상**: Exit | 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(WACC\) | 가중평균자본비용 | 기업·프로젝트 할인율 근사 |
| \(FCF\) | 잉여현금흐름 | 투자자에게 가용한 현금 |
|  \(\sum_{t\)  |  \sum t  | \(\sum t\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(FCF_t}\)  |  FCF t  | \(FCF t\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(T\) | 기간 | 마지막 CF 시점 |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |

**5,000억**, \(T=5\), \(r=40\%\), \(M=10\) → **Post today** **근사** **계산** (학습자 **엑셀**).
