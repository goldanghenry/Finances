...
|             \(bar\)             | bar | bar이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|   \(c\)   | 지출 | 기간 총 현금 유출 |
|   \(Y\)   | 소득 | 기간 총 실수령·매출 등 |
|   \(T\)   | 기간 | 마지막 CF 시점 |
\[
...
|            \(G\)            | G | G이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
Y^* = \frac{1}{1-c}\left(\bar{C} + \bar{I} + G - cT\right)
\]
...
**읽는 법**: **Y**와 **G**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
...
**읽는 법**: **PV**와 **M**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **PV**, **M**, **P**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
| \(c\) (MPC) | \(1/(1-c)\) | 해석 |
| ------------- | ------------- | -------------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| 0.6 | 2.5 | 저소비·고저축 |
| 0.8 | 5.0 | 교과서 예 |
| 0.9 | 10.0 | 승수 크지만 **누수·크라우딩**에 취약 |

**비교정태**: \(c \uparrow \Rightarrow\) 승수 \(\uparrow\). \(\b| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(C\) | 지출 | 기간 총 현금 유출 |
|            \(I\)            | I | I이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|            \(G\)            | G | G이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
ar{C}, \bar{I}, G \uparrow \Rightarrow Y^* \uparrow\) 및 \(C=c(Y^*-T) \uparrow\).

### 6.2 IS 곡선 유도

투자을 **금리 함수**로: \(I = \bar{I} - b| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
| \(C\) | 지출 | 기간 총 현금 유출 |
| \(T\) | 기간 | 마지막 CF 시점 |
...
|            \(G\)            | G | G이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
Y = \underbrace{\frac{1}{1-c}(\bar{C} - cT + \bar{I} + G)}_{\text{자율지출×승수}} - \underbrace{\frac{b}{1-c}}_{\text{기울기}} r
\]
...
/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
**IS의 성질**:
- \(r \uparrow \Rightarrow Y \downarrow\) — **우하향**
- \(G \uparrow\) 또는 \(T \downarrow\) → **절편 ↑** → IS **우측** 이동
- \(\bar{I} \uparrow\) (낙관) → IS **우측*| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
...
**그래프**: \(r\)–\(Y\) 평면에 직선(또는 볼록 곡선). **45°선은 사용하지 않음** — IS-LM은 \(r\)–\(Y\) 공간.
...
입한다.
\[
r = \frac{k}{h} Y - \frac{1}{h}\| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |
| \(P\) | 포트 규모 | 가상 포트폴리오 규모(만 원) |
...
**읽는 법**: **Y**와 **M**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **Y**, **M**, **P**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.

**LM의 성질**:
- \(Y \uparrow \Rightarrow r \uparrow\) — **우상향**
- \(M \uparrow\) 또는 \(P | 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |
| \(P\) | 포트 규모 | 가상 포트폴리오 규모(만 원) |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
...
|            \(I\)            | I | I이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|            \(G\)            | G | G이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\downarrow\) → **절편 ↓** → LM **우하** 이동
- \(k/h\) 가 **기울기** — 화폐 수요의 **소득·금리 민감도**에 의존

**실질화폐**: 균형은 **\(M/P\)** — [macro-02](macro-02-money-inflation.md)의 **명목 vs 실질**과 직결.
...
\[
Y^* = \frac{1}{1-c}\left(\bar{C} - cT + \bar{I} + G - br\right), \quad
\frac{M}{P} = k Y^* - h r^*
\]
...
화 확대** (\(M \uparrow\)): LM → → \(Y \uparrow, r \downarrow\).

**정책 조합**: 재정·통화 **동시 확대** (2020 유형) → \(Y\) ↑는 크지만 \(r\) **방향은 상쇄** 가능 — **채권·은행** 이익·손해 갈림.

### 6.5 IS-LM에서 AD 곡선 유도

**물가 \(P \uparrow\)** → 실질화폐 \(M/P | 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
| \(P\) | 포트 규모 | 가상 포트폴리오 규모(만 원) |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |

\downa| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
| \(P\) | 포트 규모 | 가상 포트폴리오 규모(만 원) |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |

rrow\) → LM **좌측** 이동 → \(r \uparrow, Y \downarrow\).
...
**읽는 법**: **Y**와 **P**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
1. **정의**: **Y**, **P**, **M**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
...
### 6.6 AD-AS: 단기 우상향·장기 | 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(Y_n\)  |  Y n  | Y n이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(P\) | 포트 규모 | 가상 포트폴리오 규모(만 원) |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(PV\) | 현재가치 | 오늘 시점으로 환산한 금액 |
|  \(Y_n\)  |  Y n  | Y n이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |
...
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
 ...
 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
\(Y > Y_n\) → **과열** → 임금·원가 압력 → \(P \uparrow\).  
\(Y < Y_n\) → **슬랙** → \(P\) 상승 둔화.
...
**읽는 법**: **PV**와 **Y_n**의 관계를 위 식으로 쓴다. 경제·재무 해석은 변수표 「이 식에서 의미」와 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 기호 예제를 맞춘다.
**유도 (L4)**:
...
**극단| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(Y_d\)  |  Y d  | Y d이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|            \(G\)            | G | G이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |
| \(T\) | 기간 | 마지막 CF 시점 |

**: LM **수직**(고전) → \(G \uparrow\) 가 **전부 \(| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(Y_d\)  |  Y d  | Y d이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |
...
\(r\) 이 **최소 \(r_{\min}\)** (영%| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(Y_d\)  |  Y d  | Y d이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |
...
- **투기화폐 수요** 무한대 근사 → \(h \to \infty\)| 기호 | 이름 | 이 식에서 의미 |
|       ------       | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(Y_d\)  |  Y d  | Y d이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(M\) | 월 실수령 | 가계 교육용 월 세후 소득 기호 |
| \(T\) | 기간 | 마지막 CF 시점 |
