...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
|  \(P_0\)  |  P_0  | 본문 §4·위 식 맥락 참고 |
|  \(t\)  |  t  | 마지막 CF 시점 |
|  \(infty\)  |  infty  | 본문 §4·위 식 맥락 참고 |
|  \(E\)  |  E  | 본문 §4·위 식 맥락 참고 |
|  \(D_\)  |  D_  | 본문 §4·위 식 맥락 참고 |
|  \(r\)  |  r  | 기간당 이자·요구수익률 |
\[
...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(P_0\) | P 0 | 본문 §4·위 식 맥락 참고 |
| \(D_1}\) | D 1 | 본문 §4·위 식 맥락 참고 |
| \(D_1\) | D 1 | 본문 §4·위 식 맥락 참고 |
| \(D_0\) | D 0 | 본문 §4·위 식 맥락 참고 |
...
**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
교육용 할인율(단순화): \(r_i \approx R_f + \beta_i \cdot ERP\).

**유도 직관**: CAPM은 **균형**에서 “시장 포트폴리오”가 **체계적 위험** \( \beta \) 에 대해 **ERP** 만큼 초과수익을 요구한다고 **가정**. Gordon에 대입:
...
\[
P = \frac{D_1}{R_f + \beta ERP - g}
\]


**읽는 법**: 위 식의 기호는 바로 위 변수표와 같다. 숫자는 [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 교육용 기호(M·P·PV 등)로 대입한다.
**Fed funds↑ → \(R_f\uparrow\)** (전달 가정) → **\(P\downarrow\)**. **성장주**는 보통 **\(\beta > 1\)** → **민감도 증폭**.

### 6.3 Fed funds 전파 — 교육용 2단계

1. **정책**: \(i_{ff}\) (Fed funds target) ↑  
2. **국채**: \(R_f(\tau) \approx \) 단기는 **\(i_{ff}\)** 에 **앵커**, 장기는 **\(E[\sum i_{ff,s}] + \text{term premium}\)**

**성장주 할인율**: 장기 CF → **장기 \(R_f\)** + **ERP** — **“2년물만”** 보면 부족, **10년·실질금리**도 추적.

**QQQ**: 대형 **성장·기술** 비중 → **높은 g·높은 \(\be| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(r_{nom}\) | r nom | 본문 §4·위 식 맥락 참고 |
| \(r_{real}\) | r real | 본문 §4·위 식 맥락 참고 |

ta\)** → **\(r\)** 민감. **QLD**: 일 수익 **2×** → **\(r\)** 충격의 **일별** 증폭 — [leveraged-etf-qqq-qld.md](../04-portfolio/leveraged-etf-qqq-qld.md).

### 6.4 Fisher 방정식과 실질수익

**정확**: \((1+r_{nom})(1+r_{real}) = 1+\pi\) (근사 아님).
...
|------|------|----------------|
| \(R_{real}\) | R real | 본문 §4·위 식 맥락 참고 |
| \(R_{after\) | R after | 본문 §4·위 식 맥락 참고 |
...
### 6.6 ISA·IRP 세후 실질수익 | 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(R_{after\) | R after | 본문 §4·위 식 맥락 참고 |
| \(R_{nom}\) | R nom | 본문 §4·위 식 맥락 참고 |
| \(\tau_{ISA}\) | \tau ISA | 본문 §4·위 식 맥락 참고 |
| \(R\) | R | 기간당 이자·요구수익률 |
...
**10년 복리** (가상, 세후 실질 6% vs 8.7%): 1,000만 → **약 1,791만 vs 2,337만** — **계좌 선택**이 **매크로 수익**과 **동급** 중요 — [isa-irp-pension-tax.md](../06-korea-policy/tax/isa-irp-pension-tax.md).
