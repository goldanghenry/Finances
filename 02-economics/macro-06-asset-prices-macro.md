...
|   \(P_0\)   | P 0 | P 0이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|   \(t\)   | 기간 | 마지막 CF 시점 |
|              \(infty\)              | infty | infty이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
|        ------        | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(P_0\)  |  P 0  | P 0이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(D_1}\)  |  D 1  | \(D 1\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(D_1\)  |  D 1  | D 1이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(D_0\)  |  D 0  | D 0이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
| 기호 | 이름 | 이 식에서 의미 |
|        ------        | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(R_i\)  |  R i  | R i이(가) 이 식에서 맡는 역할(§4·본문 참고) |
| \(R_f\) | 무위험금리 | 국채·예금 등 기준 금리 |
|  \(eta_i\)  |  eta i  | eta i이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(R_m\)  |  R m  | R m이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|             \(E\)             | E | E이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
**유도 (L4)**:
1. **정의**: **R_i**, **R_f**, **eta_i**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
...
**읽는 법**: 시장 초과수익에 대한 민감도가 **β**다. **R_f**·**ERP**와 함께 요구수익 **r**을 구성한다. [DEPTH-STANDARD](../docs/DEPTH-STANDARD.md) 참고.
**유도 (L4)**:
1. **정의**: **ERP**, **D_1**, **R_f**를 동일 시점·동일 통화로 맞춘다. — 단위 불일치면 식이 무의미해진다.
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
...
|        ------        | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(r_{nom}\)  |  r nom  | \(r nom\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(r_{real}\)  |  r real  | \(r real\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
...
|        ------        | ------ | ------이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(R_{real}\)  |  R real  | \(R real\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(R_{after\)  |  R after  | \(R after\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
\[
...
|  \(R_{after\)  |  R after  | \(R after\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(R_{nom}\)  |  R nom  | \(R nom\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|  \(\tau_{ISA}\)  |  \tau ISA  | \(\tau ISA\)이(가) 이 식에서 맡는 역할(§4·본문 참고) |
|            \(R\)            | R | 기간당 이자·요구수익률 |
(가상 프레임)
