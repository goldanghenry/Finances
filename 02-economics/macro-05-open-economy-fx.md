...
\(P^*\): 외국 물가지수, \(P\): 국내. \(q \uparrow\) → **실질 원화 약세** → (ceteris paribus) **수출 경쟁력↑**, **수입 부담↑**.
...
4. **거래비용·관세** — [micro-04](micro-04-welfare-externalities.md) 관세가 **\(P_d = P_w + t\)** 로 **실질 q** 왜곡  
...
**2020s 한국**: 미국 금리 급등 구간에서 \(i-i^*\)가 **음수** 또는 **작은 양수**여도 **원화 약세**가 지속될 수 있음 — **rp↑**(지정학·반도체·외채·글로벌 달러 수요). “금리차 = 환율” **단순 회귀**는 실패한다.
...
**부호 주의**: 교재마다 \(e\)를 “외화 per 본국” vs “본국 per 외화”로 정의 — 본 문서는 **\(e\) = KRW/USD** (원화 per 1달러). \(e\uparrow\) = **원화 약세**.

**고정환율 + 자본이동**: M↑ → **준비금 축적**, \(r\)은 **세계 금리**에 **고정** — **통화정책 무효**. 재정 G↑ → \(Y\uparrow\) **유효** (LM이 **수평**에 가깝게).

### 6.6 Marshall-Lerner·J-curve
...
B는 **VOO 1억 원**(언헷지) + **국내 수출 ETF 1억 원** 보유.
