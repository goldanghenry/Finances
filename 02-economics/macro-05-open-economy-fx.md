...
명목 **5%** 약세, 국내 물가 **3%p** 더 오르면 실질 약세는 **약 2%** — “원
...
2. **Balassa-Samuelson**: 저소득국 **생산성 추격** → \(P/P^*\) 상승 → **명목 e
|   이름   |   이 식에서 의미   | §4 용어·식 맥락에서 확인 |
| 기호 | 이름 | 이 식에서 의미 |
|    ------    | ------ | 위 식의 ------ |
...
4. **거래비용·관세** — [micro-04](micro-04-welfare-e
|   이름   |   이 식에서 의미   | §4 용어·식 맥락에서 확인 |
| 기호 | 이름 | 이 식에서 의미 |
|    ------    | ------ | 위 식의 ------ |
...
한국 금리 \(i\) > 미국 \(i^*\) 이면, **균형**에서는 **원화 예상 절상**(또는 달러 프리미엄) —
|   이름   |   이 식에서 의미   | §4 용어·식 맥락에서 확인 |
| 기호 | 이름 | 이 식에서 의미 |
|    ------    | ------ | 위 식의 ------ |
| \(r\) | 할인율·수익률 | 기간당 이자·요구수익률 |
...
2. **식 변형**: 양변을 정리해 목표 변수를 한쪽에 둔다. — 할인·복리는 **시점 이동**이 핵심이다.
3. **해석**: 부호·크기가 경제 직관과 맞는지 확인한다. — 극단값에서 단조성·한계를 점검한다.
**2020s 한국**: 미국 금리 급등 구간에서 \(i-i^*\)가 **음수** 또는 **작은
|   이름   |   이 식에서 의미   | §4 용어·식 맥락에서 확인 |
| 기호 | 이름 | 이 식에서 의미 |
|    ------    | ------ | 위 식의 ------ |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
...
- **통화 M↑**: LM 우측 → \(r\downarrow\) → **자본 유출** → **e↓
|   이름   |   이 식에서 의미   | §4 용어·식 맥락에서 확인 |
| 기호 | 이름 | 이 식에서 의미 |
|    ------    | ------ | 위 식의 ------ |
| \(Y\) | 소득 | 기간 총 실수령·매출 등 |
