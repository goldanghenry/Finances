...
| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
|  \(CS\)  |  CS  | 본문 §4·위 식 맥락 참고 |
|  \(int\)  |  int  | 본문 §4·위 식 맥락 참고 |
|  \(Q\)  |  Q  | 본문 §4·위 식 맥락 참고 |
|  \(bigl\)  |  bigl  | 본문 §4·위 식 맥락 참고 |
|  \(a\)  |  a  | 본문 §4·위 식 맥락 참고 |
|  \(b\)  |  b  | 본문 §4·위 식 맥락 참고 |
|  \(P\)  |  P  | 가상 포트폴리오 규모(만 원) |
|  \(bigr\)  |  bigr  | 본문 §4·위 식 맥락 참고 |
\[
...
세수| 기호 | 이름 | 이 식에서 의미 |
|------|------|----------------|
| \(Q_t\) | Q t | 본문 §4·위 식 맥락 참고 |
| \(T\) | 기간 | 마지막 CF 시점 |
| \(Q\) | Q | 본문 §4·위 식 맥락 참고 |

 \(T = t \cdot Q_t\). **DWL** (교육용 삼각형):
...
보조금으로 소비자 체감가격이 20% 하락, 수요탄력성 \(|\varepsilon|=1.2\), 판매 10만 대→12만 대. 한계사회비용(혼잡·전력 피크·재활용 미반영) 때문에 **마지막 2만 대**는 \(MB < MSC\).

가상 DWL 삼각형: 보조금 단위 300만 원 × 비효율 수량 2만 대 × \(\frac{1}{2}\) ≈ **300억 원** 규모(교육용).
